...
'               al ultimo FMS con fecha <= Dia (el FMS del mismo dia aun no
'               refleja la marca; el peso es siempre pre-marca). Fecha, valor
...
Public Sub ActualizarTodo()
    Dim inf As String, t0 As Single, t1 As Single, tiempos As String
    t0 = Timer: t1 = Timer
    inf = CargarFMS_Interno(False) & vbCrLf
    tiempos = "FMS " & Format$(Timer - t1, "0") & " s": t1 = Timer
    inf = inf & CargarMarcas_Interno() & vbCrLf
    tiempos = tiempos & " | Marcas " & Format$(Timer - t1, "0") & " s": t1 = Timer
    inf = inf & Procesar_Interno()
    tiempos = tiempos & " | Procesar " & Format$(Timer - t1, "0") & " s": t1 = Timer
    FechaReporteHoy
    Application.Calculation = xlCalculationAutomatic
    Application.Calculate
    tiempos = tiempos & " | Recalculo " & Format$(Timer - t1, "0") & " s"
    Debug.Print inf
    Debug.Print tiempos
    MsgBox inf & vbCrLf & vbCrLf & tiempos & " (total " & Format$(Timer - t0, "0") & " s). Detalle en Inmediato (Alt+F11, Ctrl+G).", _
...
Private Function HojaEventos() As Worksheet
    Set HojaEventos = HojaDatos(H_EVT, "Eventos - una fila por marca valida ordenada por fondo y Dia; peso al ultimo FMS con fecha <= Dia, que aun no refleja la marca (I-K, M-N los ubica Procesar); peso, contribucion y ln(1+r) en formulas de la fila.", _
...
Private Function CargarFMS_Interno(todo As Boolean) As String
    ...
    Dim dExist As Object, ultB As Long, colF As Variant
    Set dExist = CreateObject("Scripting.Dictionary")
    ultB = UltimaFila(ws, BF_FECHA)
    If ultB >= F_INI Then
        colF = ws.Range(ws.Cells(F_INI, BF_FECHA), ws.Cells(ultB, BF_FECHA)).Value
        For i = 1 To UBound(colF, 1)
            If IsDate(colF(i, 1)) Then dExist(CLng(CDate(colF(i, 1)))) = 1
        Next i
    End If

    Application.StatusBar = "Buscando FMS nuevos en la carpeta..."
...
Private Function CargarMarcas_Interno() As String
    ...
    Application.DisplayAlerts = False
    Application.Calculation = xlCalculationManual
    Application.StatusBar = "Leyendo Marcas.xlsx..."
    ...
    For h = LBound(hojas) To UBound(hojas)
        ...
        If Not wsM Is Nothing Then
            ...
            For i = 1 To N_FONDOS
                ...
                If col > 0 Then
                    Dim omit As String: omit = vbNullString
                    k = LeerBloqueMarcas(wsM, col, i, cat, ultFila, ws, fila, nombre, omit)
                    tot = tot + k
                    inf = inf & "Hoja " & wsM.Name & " | " & cat(i, 1) & ": " & k & " marcas (col " & _
                          Split(wsM.Cells(1, col).Address, "$")(1) & ")" & _
                          IIf(omit <> "", " | NO LEIDAS: " & omit, "") & vbCrLf
                End If
            Next i
        End If
    Next h
...
Private Function Procesar_Interno() As String
    ...
    Application.ScreenUpdating = False: Application.EnableEvents = False
    Application.Calculation = xlCalculationManual
    Application.StatusBar = "Procesando eventos..."
    ...
    For p = 1 To n
        ...
        For i = nF To 1 Step -1
            If fechasB(i) <= CLng(dia) Then fT = fechasB(i): encontrado = True: Exit For   ' ultimo FMS <= Dia (aun no refleja la marca)
        Next i
        If encontrado Then
            salida(p, 9) = CDate(fT)
    ...
    Next p
    Dim ultBD As Long: ultBD = UltimaFila(wsB, BF_FECHA)
    If ultBD < F_INI Then ultBD = F_INI
    EscribirMensual cat, mesMin, mesMax, ultE, ultBD
    EscribirMatriz cat, mesMax, ultE, ultBD
...
Private Sub EscribirMatriz(cat As Variant, mesMax As Date, ultE As Long, ultBD As Long)
    ...
    eS = EQ & "$B$" & F_INI & ":$B$" & ultE: eD = EQ & "$E$" & F_INI & ":$E$" & ultE: eMo = EQ & "$D$" & F_INI & ":$D$" & ultE
    eM = EQ & "$R$" & F_INI & ":$R$" & ultE: eP = EQ & "$P$" & F_INI & ":$P$" & ultE
    eCF2 = EQ & "$Q$" & F_INI & ":$Q$" & ultE: eL = EQ & "$T$" & F_INI & ":$T$" & ultE
    Dim bF As String: bF = BQ & "$A$" & F_INI & ":$A$" & ultBD
                ...
                Select Case b
                    Case 0      ' retorno del bloque = contribucion F1 del mes / peso F1 del bloque al FMS anterior al mes
                        ws.Cells(r, 2 + c).Formula = "=IF(COUNTIFS(" & eM & "," & mCel & crit & ")=0,"""",IFERROR(SUMIFS(" & eP & "," & eM & "," & mCel & crit & ")/" & _
                            PesoBloqueHasta("F1", CStr(mon(k)), "MAXIFS(" & bF & "," & bF & ",""<""&" & mCel & ")", BQ, ultBD) & ",""""))"
                    Case 1
...
Private Sub EscribirMensual(cat As Variant, mesMin As Date, mesMax As Date, ultE As Long, ultBD As Long)
    Dim ws As Worksheet, fila As Long, m As Date, i As Long, k As Long
    Set ws = HojaMensual()
    LimpiarDatos ws, MS_NCOL
    Dim EQ As String, BQ As String
    EQ = H_EVT & "!": BQ = H_BDFMS & "!"
    ' rangos exactos (Procesar los reconstruye en cada corrida): recalculo mucho mas rapido
    Dim eS As String, eD As String, eM As String, eP As String, eCF2 As String, eL As String
    eS = EQ & "$B$" & F_INI & ":$B$" & ultE: eD = EQ & "$D$" & F_INI & ":$D$" & ultE
    eM = EQ & "$R$" & F_INI & ":$R$" & ultE: eP = EQ & "$P$" & F_INI & ":$P$" & ultE
    eCF2 = EQ & "$Q$" & F_INI & ":$Q$" & ultE: eL = EQ & "$T$" & F_INI & ":$T$" & ultE
    Dim bF As String, bFo As String, bS As String, bMo As String, bV As String, bA As String
    bF = BQ & "$A$" & F_INI & ":$A$" & ultBD: bFo = BQ & "$B$" & F_INI & ":$B$" & ultBD
    bS = BQ & "$C$" & F_INI & ":$C$" & ultBD: bMo = BQ & "$E$" & F_INI & ":$E$" & ultBD
    bV = BQ & "$G$" & F_INI & ":$G$" & ultBD: bA = BQ & "$H$" & F_INI & ":$H$" & ultBD
...
Private Function PesoBloqueHasta(fondo As String, moneda As String, celFecha As String, BQ As String, ultBD As Long) As String
    Dim crit As String
    If moneda <> "Total" Then crit = "," & BQ & "$E$" & F_INI & ":$E$" & ultBD & ",""" & moneda & """"
    PesoBloqueHasta = "(SUMIFS(" & BQ & "$G$" & F_INI & ":$G$" & ultBD & "," & BQ & "$A$" & F_INI & ":$A$" & ultBD & "," & celFecha & "," & _
                      BQ & "$B$" & F_INI & ":$B$" & ultBD & ",""" & fondo & """" & crit & ")/AVERAGEIFS(" & BQ & "$H$" & F_INI & ":$H$" & ultBD & "," & _
                      BQ & "$A$" & F_INI & ":$A$" & ultBD & "," & celFecha & "," & BQ & "$B$" & F_INI & ":$B$" & ultBD & ",""" & fondo & """))"
End Function
...
Private Function LeerBloqueMarcas(wsM As Worksheet, col As Long, i As Long, cat As Variant, _
                                  ...
                                  archivo As String, ByRef omit As String) As Long
    ...
    Dim d As Date, esFecha As Boolean
    cVA = col + MK_OFF_VA
    For k = 0 To 7
    ...
    For r = MK_FILA_INI To ultFila
        ...
        ElseIf VarType(v) = vbString Then
            s = UCase$(Trim$(v))
            If s = vbNullString Then GoTo Sig
            If s = cat(i, 2) Or s = "F1" Or s = "F2" Or s = "F3" Or Left$(s, 6) = "MONEDA" Then Exit For
            If IsDate(v) Then d = CDate(v): esFecha = True                ' fecha escrita como texto
        ElseIf IsNumeric(v) Then
            If v >= 36526 And v <= 55000 Then d = CDate(v): esFecha = True   ' serial de fecha sin formato
        End If
        If Not esFecha Then
            omit = omit & IIf(omit = "", "", ", ") & "fila " & r & " ('" & Left$(CStr(v), 15) & "')"
            GoTo Sig
        End If
        If Year(d) < 2000 Or Year(d) > 2100 Then
            omit = omit & IIf(omit = "", "", ", ") & "fila " & r & " (año " & Year(d) & ")"
        Else
            If True Then
                ...
                ws.Cells(fila, BM_DIA).Value = d
                If VarType(ee) = vbDate Then
                    ws.Cells(fila, BM_EEFF).Value = ee
                ElseIf IsDate(ee) Then
                    ws.Cells(fila, BM_EEFF).Value = CDate(ee)
                    motivo = Trim$(Replace(Replace(motivo, "sin EEFF", ""), "; ", ""))
                End If
