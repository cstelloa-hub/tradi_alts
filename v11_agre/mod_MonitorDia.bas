...
'    Eventos    una fila por marca valida ordenada por Dia, con el peso
...
Private Function Cfg() As Worksheet
    ...
    ws.Columns(2).ColumnWidth = 30: ws.Columns(3).ColumnWidth = 34
...
Private Function Procesar_Interno() As String
    ...
    Dim sinPeso As Long, sinFMS As Long, remarcas As Long
    For p = 1 To n
        r = idx(p)
        ...
        If IsDate(eeff) Then
            ...
            End If
        End If
        If va = 0 Then flags = flags & IIf(flags = "", "", "; ") & "Var Adj = 0"
    ...
    inf = "Procesar: " & n & " eventos (" & remarcas & " re-marcas, " & sinFMS & " sin FMS, " & _
          sinPeso & " sin posicion). Mensual " & Format$(mesMin, "mmm-yy") & " a " & Format$(mesMax, "mmm-yy") & "."
...
Private Sub EscribirMatriz(cat As Variant, mesMax As Date)
    ...
    EQ = H_EVT & "!": BQ = H_BDFMS & "!"
    Dim eS As String, eD As String, eMo As String, eM As String, eP As String, eQ As String, eL As String
    eS = EQ & "$B$" & F_INI & ":$B$" & F_MAX: eD = EQ & "$E$" & F_INI & ":$E$" & F_MAX: eMo = EQ & "$D$" & F_INI & ":$D$" & F_MAX
    eM = EQ & "$R$" & F_INI & ":$R$" & F_MAX: eP = EQ & "$P$" & F_INI & ":$P$" & F_MAX
    eQ = EQ & "$Q$" & F_INI & ":$Q$" & F_MAX: eL = EQ & "$T$" & F_INI & ":$T$" & F_MAX
        ...
        For i = 1 To N_FONDOS
            ...
            For c = 1 To nMes
                ...
                Select Case b
                    ...
                    Case 2
                        ws.Cells(r, 2 + c).Formula = "=IF(COUNTIFS(" & eS & "," & key & "," & eM & "," & mCel & "," & eQ & ","">=-1"")=0,""""," & _
                            "SUMIFS(" & eQ & "," & eS & "," & key & "," & eM & "," & mCel & ")*10000)"
                End Select
            Next c
        Next i
        For k = 0 To 2                                                    ' totales
            ...
            For c = 1 To nMes
                ...
                Select Case b
                    ...
                    Case 2
                        ws.Cells(r, 2 + c).Formula = "=IF(COUNTIFS(" & eM & "," & mCel & crit & "," & eQ & ","">=-1"")=0,"""",SUMIFS(" & eQ & "," & eM & "," & mCel & crit & ")*10000)"
                End Select
            Next c
...
Private Sub EscribirMensual(cat As Variant, mesMin As Date, mesMax As Date)
    ...
    EQ = H_EVT & "!": BQ = H_BDFMS & "!"
    Dim eS As String, eD As String, eM As String, eP As String, eQ As String, eL As String
    ...
    Do While m <= mesMax
        For i = 1 To N_FONDOS
            ...
            ws.Cells(fila, 8).Formula = "=SUMIFS(" & eQ & "," & eS & ",$B" & fila & "," & eM & ",$A" & fila & ")*10000"
        ...
        For k = 0 To 2
            ...
            If k < 2 Then
                ...
                ws.Cells(fila, 8).Formula = "=SUMIFS(" & eQ & "," & eD & ",$D" & fila & "," & eM & ",$A" & fila & ")*10000"
            ...
            Else
                ...
                ws.Cells(fila, 8).Formula = "=SUMIFS(" & eQ & "," & eM & ",$A" & fila & ")*10000"
...
Public Sub CrearResumen()
    ...
    With ws.Range("C3")
        ...
        .Validation.InputMessage = "Cualquier fecha. Posicion: ultimo FMS <= fecha. Retornos y contribucion: marcas con Dia <= fecha."
    End With
    Dim wsB As Worksheet: Set wsB = ThisWorkbook.Worksheets(H_BDFMS)
    If UltimaFila(wsB, BF_FECHA) >= F_INI Then ws.Range("C3").Value = wsB.Cells(UltimaFila(wsB, BF_FECHA), BF_FECHA).Value
    ...
    Dim eS As String, eD As String, eMo As String, eP As String, eQ As String, eL As String
