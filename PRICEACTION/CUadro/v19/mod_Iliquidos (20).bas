...
Private Const VER As String = "2.14"
...
Public Sub CrearConfig()
    ...
    Map ws, 48, "Fila de la cabecera", "1"
    Map ws, 49, "Primera fila de datos", "2"
    Map ws, 50, "Columna Dia", ""
    Map ws, 51, "Columna Nombre del fondo", ""
    Map ws, 52, "Columna Var Ajustada", ""
...
Public Sub CrearInstrumentos()
    ...
    ws.Range("L1").Value = "Nombre en Marcas (solo si no calza)"
...
Public Sub CargarMarcas()
    ...
    ' Se buscan en Marcas los instrumentos que NO estan en el vector, que son
    ' justamente los fondos. El nombre por defecto es el de la columna B; si
    ' en Marcas se llama distinto, lo escribes en la columna L y manda ese.
    If Not HojaExiste(SH_INS) Then
        Err.Raise vbObjectError + 31, , "No existe la hoja " & SH_INS & "."
    End If
    Set wsIns = ThisWorkbook.Worksheets(SH_INS)
    Set mapa = NuevoDic()
    Set codigos = NuevoDic()
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 And Len(Trim$(Txt(wsIns.Cells(i, 3).Value))) > 0 Then
            If InStr(1, Txt(wsIns.Cells(i, 11).Value), "REVISAR", vbTextCompare) > 0 _
               Or Len(Trim$(Txt(wsIns.Cells(i, 12).Value))) > 0 Then
                nom = Trim$(Txt(wsIns.Cells(i, 12).Value))
                If Len(nom) = 0 Then nom = Trim$(Txt(wsIns.Cells(i, 2).Value))
                If Len(nom) = 0 Then nom = Trim$(Txt(wsIns.Cells(i, 10).Value))
                If Len(nom) = 0 Then
                    nSinNombre = nSinNombre + 1
                Else
                    mapa(NormTxt(nom)) = cod
                    codigos(cod) = nom
                End If
            End If
        End If
    ...
    If mapa.Count = 0 Then
        msg = "No hay ningun instrumento que buscar en Marcas." & vbCrLf & vbCrLf
        msg = msg & "Se buscan los que en Instrumentos tienen categoria y ademas"
        msg = msg & vbCrLf & "no aparecen en el vector (columna K), o tienen algo"
        msg = msg & vbCrLf & "escrito en la columna L."
        Err.Raise vbObjectError + 32, , msg
    End If
    ...
    If colNom = 0 Then colNom = BuscaCol(v, cab, "FONDO|NOMBRE|DESCRIPCION", filaCab)
    ...
    If colDia = 0 Or colNom = 0 Or colVar = 0 Then
        ...
        msg = msg & "   Nombre: " & LetraODash(colNom)
    ...
    For Each k In mapa.Keys
        ...
        wsFon.Cells(fila, 2).Value = CStr(codigos(cod))
        ...
        Else
            ...
            wsFon.Cells(fila, 10).Value = "REVISAR: no encontre marcas con ese nombre"
            wsFon.Range(wsFon.Cells(fila, 1), wsFon.Cells(fila, 10)).Interior.Color = RGB(255, 235, 156)
        End If
    Next k
    ...
    msg = msg & "Fondos en el puente: " & mapa.Count & vbCrLf
    msg = msg & "Marcas usadas: " & Format$(nUsadas, "#,##0") & vbCrLf
    msg = msg & "Corte: " & Format$(CDate(dCorte), "dd/mm/yyyy") & vbCrLf
    If nSinNombre > 0 Then
        msg = msg & "REVISAR: " & nSinNombre & " con codigo pero sin nombre en la col B" & vbCrLf
    End If
    If nSinMarca > 0 Then
        msg = msg & "REVISAR: " & nSinMarca & " sin ninguna marca (mira la col J)" & vbCrLf
    End If
    If raros > 0 Then
...
Private Sub ArmarCuadro(ins As Object, orden As Variant, dCorte As Double)
        ...
        For ic = LBound(cats) To UBound(cats)
            ...
            For i = LBound(orden) To UBound(orden)
                ...
                If arr(3) = mon And arr(2) = cat And Not arr(4) Then
                    r = r + 1
                    FilaInstrumento ws, r, mon, cat, CStr(arr(0)), CStr(arr(1)), "INS", fondos
                End If
            Next i
        Next ic
    Next im
    ...
    If hayFuera Then
        ...
        For i = LBound(orden) To UBound(orden)
            ...
            If arr(4) Then
                r = r + 1
                FilaInstrumento ws, r, "", "", CStr(arr(0)), CStr(arr(1)), "OUT", fondos
            End If
        Next i
    End If
...
Private Sub FilaInstrumento(ws As Worksheet, r As Long, mon As String, cat As String, _
                            cod As String, nom As String, tipo As String, fondos As Object)
    ...
    If esFondo Then
        ...
        For j = 0 To N_VENT - 1
            If Not IsEmpty(f(1 + j)) Then ws.Cells(r, CC_W1 + j).Value = f(1 + j)
        Next j
    Else
