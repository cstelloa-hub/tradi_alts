...
Public Sub CargarMarcas()
    ...
    cDia = Letra(Txt(wsCfg.Range("C50").Value), 0)
    cNom = Letra(Txt(wsCfg.Range("C51").Value), 0)
    cVar = Letra(Txt(wsCfg.Range("C52").Value), 0)
    If cDia = 0 Then cDia = BuscaCol(v, cab, "DIA|FECHA")
    If cNom = 0 Then cNom = BuscaCol(v, cab, "FONDO|NOMBRE|DESCRIPCION")
    If cVar = 0 Then cVar = BuscaCol(v, cab, "VAR AJ|VARIACION AJ|VARAJ")
    If cDia = 0 Or cNom = 0 Or cVar = 0 Then
        msg = "No pude ubicar las columnas de Marcas." & vbCrLf & vbCrLf
        msg = msg & "Dia: " & LetraODash(cDia)
        msg = msg & "   Nombre: " & LetraODash(cNom)
        msg = msg & "   Var Adj: " & LetraODash(cVar) & vbCrLf & vbCrLf
    ...
    For i = fil0 To nFilas
        If EsNumero(v(i, cDia)) Then
            dFe = CDbl(v(i, cDia))
            cla = NormTxt(Txt(v(i, cNom)))
            If mapa.Exists(cla) Then
                If EsNumero(v(i, cVar)) Then
                    vAdj = CDbl(v(i, cVar))
    ...
    msg = "v2.11 - CargarMarcas terminado en " & Format$(Timer - mT0, "0.0") & " s."
    msg = msg & vbCrLf & vbCrLf & mPasos & vbCrLf
    msg = msg & "Columnas: Dia " & NumALetra(cDia)
    msg = msg & " . Nombre " & NumALetra(cNom)
    msg = msg & " . Var Adj " & NumALetra(cVar) & vbCrLf
...
Public Sub CargarVectores()
    ...
    t = Timer - mT0
    msg = "v1.0 - CargarVectores terminado en " & Format$(t, "0.0") & " s"
