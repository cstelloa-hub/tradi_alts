...
'   CrearFondos      crea el puente codigo SBS <-> nombre en Marcas.
...
'   6. CrearFondos          -> codigo SBS y nombre en Marcas de cada fondo
'   7. Ruta de Marcas en Config C46, y CargarMarcas
'   8. Recalcular           -> hojas Retornos y CUADRO
'
...
Private Const VER As String = "2.12"
...
Public Sub CargarMarcas()
    ...
    Paso "Leyendo el puente de la hoja Fondos"
    If Not HojaExiste(SH_FON) Then
        Err.Raise vbObjectError + 31, , "No existe la hoja " & SH_FON & ". Corre CrearFondos."
    End If
    Set wsFon = ThisWorkbook.Worksheets(SH_FON)
    Set mapa = NuevoDic()
    ult = wsFon.Cells(wsFon.Rows.Count, 1).End(xlUp).Row
    For i = 2 To ult
        nom = Trim$(Txt(wsFon.Cells(i, 2).Value))
        If Len(SinGuiones(Txt(wsFon.Cells(i, 1).Value))) > 0 Then
            If Len(nom) = 0 Then
    ...
    If mapa.Count = 0 Then
        msg = "En la hoja " & SH_FON & " no hay ningun fondo con codigo SBS y nombre."
        msg = msg & vbCrLf & "Columna A el codigo, columna B el nombre tal cual sale en Marcas."
    ...
    If colDia = 0 Then colDia = BuscaCol(v, cab, "DIA|FECHA")
    If colNom = 0 Then colNom = BuscaCol(v, cab, "FONDO|NOMBRE|DESCRIPCION")
    If colVar = 0 Then colVar = BuscaCol(v, cab, "VAR AJ|VARIACION AJ|VARAJ")
    If colDia = 0 Or colNom = 0 Or colVar = 0 Then
        ...
        msg = msg & "Escribelas a mano en Config C50, C51 y C52."
    ...
    wsFon.Cells(1, 10).Value = "Ultima marca"
    wsFon.Cells(1, 10).Font.Bold = True
        ...
        If acum.Exists(k) Then
            ...
            For j = 0 To N_VENT - 1
                If CDbl(arr(j)) = 0 Then
                    wsFon.Cells(r, 4 + j).ClearContents
                Else
                    wsFon.Cells(r, 4 + j).Value = Exp(CDbl(arr(j))) - 1
                End If
            Next j
            wsFon.Cells(r, 10).Value = CDbl(ultMar(k))
            wsFon.Cells(r, 10).NumberFormat = "dd/mm/yyyy"
        Else
            nSinMarca = nSinMarca + 1
            For j = 0 To N_VENT - 1
                wsFon.Cells(r, 4 + j).ClearContents
            Next j
            wsFon.Cells(r, 10).Value = "REVISAR: no encontre marcas con ese nombre"
            wsFon.Cells(r, 10).Interior.Color = RGB(255, 235, 156)
        End If
    Next k
...
Private Function BuscaCol(v As Variant, cab As Long, patrones As String) As Long
    ' Busca en la fila de cabecera la primera columna cuyo texto contenga
    ' alguno de los patrones (separados por |), sin tildes ni mayusculas.
    Dim j As Long, t As String, p As Variant
    If Not IsArray(v) Then Exit Function
    If cab < LBound(v, 1) Or cab > UBound(v, 1) Then Exit Function
    For Each p In Split(patrones, "|")
        For j = LBound(v, 2) To UBound(v, 2)
            t = NormTxt(Txt(v(cab, j)))
            If Len(t) > 0 Then
...
Public Sub CrearFondos()
    ' Hoja donde pegas los RETORNOS de los fondos tradicionales, sacados del
    ' otro libro (mod_RetornoFondos, que los calcula desde Marcas).
    ' El peso NO va aqui: sale del FMS, como el de todos los demas.
    ' El cruce es por Codigo SBS. El modulo nunca escribe en esta hoja.
    Dim ws As Worksheet, cab As Variant, existia As Boolean, i As Long
    ...
    If existia Then
        ...
        MsgBox "La hoja " & SH_FON & " ya existe y no se toca." & vbCrLf & vbCrLf & _
               "Columna A el codigo SBS, columna B el nombre tal cual sale en Marcas." & vbCrLf & _
               "Los retornos los llena CargarMarcas.", vbInformation, "Fondos"
        Exit Sub
    End If

    cab = Array("Codigo SBS", "Nombre EXACTO en Marcas", "Curr. Yield", _
                "WTD", "5D", "MTD", "MayoTD", "YTD", "FY")
    ws.Range(ws.Cells(1, 1), ws.Cells(1, 9)).Value = cab
    ...
    ws.Range("A4").Value = "COMO SE USA"
    ws.Range("A4").Font.Bold = True
    ws.Range("A5").Value = "1. Columna A: el codigo SBS del fondo, el mismo de la hoja Instrumentos."
    ws.Range("A6").Value = "2. Columna B: el nombre TAL CUAL aparece en Marcas. Es el puente, tiene que calzar."
    ws.Range("A7").Value = "3. Columnas D a I: NO las toques. Las llena CargarMarcas."
    ws.Range("A8").Value = "4. El peso F1/F2 sale del FMS. No va aqui."
    ws.Range("A9").Value = "5. Curr. Yield es tuyo y puede ir vacio."
    ws.Range("A10").Value = "6. Un fondo que este aqui usa ESTOS retornos; el vector de la SBS se ignora para el."
    ws.Range("A11").Value = "7. Las tildes y mayusculas del nombre dan igual; los espacios de mas tambien."
    For i = 4 To 11
        ws.Cells(i, 1).Font.Color = RGB(128, 128, 128)
        ws.Cells(i, 1).Font.Italic = True
    Next i
    ...
    ws.Range(ws.Columns(3), ws.Columns(9)).ColumnWidth = 10
    ws.Range("A2:A500").NumberFormat = "@"
    ws.Range(ws.Cells(2, 3), ws.Cells(500, 9)).NumberFormat = "0.00%"
    CongelarPaneles ws, 1, 2
    ws.Activate

    MsgBox "Hoja " & SH_FON & " creada." & vbCrLf & vbCrLf & _
           "Escribe dos columnas por fondo:" & vbCrLf & _
           "  A: el codigo SBS (copialo de Instrumentos)" & vbCrLf & _
           "  B: el nombre tal cual sale en Marcas" & vbCrLf & vbCrLf & _
           "Despues pon la ruta del libro de Marcas en Config C46" & vbCrLf & _
           "y corre CargarMarcas. Los retornos se llenan solos.", _
