...
Public Sub CargarMarcas()
    ...
    Dim wsCfg As Worksheet, wsFon As Worksheet, wb As Workbook, ws As Worksheet
    Dim ruta As String, hoja As String
    Dim v As Variant, cab As Long, fil0 As Long
    Dim cDia As Long, cNom As Long, cVar As Long
    Dim mapa As Object, acum As Object, ultima As Object
    Dim i As Long, j As Long, ult As Long, r As Long
    Dim nom As String, cla As String
    Dim dia As Double, vAdj As Double, dCorte As Double
    Dim bases() As Double, arr As Variant, k As Variant
    Dim nFilas As Long, nUsadas As Long, nSinMarca As Long, nSinNombre As Long
    Dim yaAbierto As Boolean, raros As Long
    Dim scrPrev As Boolean, calcPrev As XlCalculation, evPrev As Boolean
    ...
    If Len(ruta) = 0 Then Err.Raise vbObjectError + 30, , _
        "Falta la ruta del libro de Marcas en Config C46."
    hoja = Trim$(Txt(wsCfg.Range("C47").Value))
    If Len(hoja) = 0 Then hoja = "Hoja 3"
    cab = CLng(NumDef(wsCfg.Range("C48").Value, 1))
    fil0 = CLng(NumDef(wsCfg.Range("C49").Value, 2))

    Paso "Leyendo el puente de la hoja Fondos"
    If Not HojaExiste(SH_FON) Then Err.Raise vbObjectError + 31, , _
        "No existe la hoja " & SH_FON & ". Corre CrearFondos."
    ...
    For i = 2 To ult
        ...
        End If
    Next i
    If mapa.Count = 0 Then Err.Raise vbObjectError + 32, , _
        "En la hoja " & SH_FON & " no hay ningun fondo con codigo SBS y nombre." & vbCrLf & _
        "Columna A el codigo, columna B el nombre tal cual sale en Marcas."
    ...
    If ws Is Nothing Then
        ...
        Err.Raise vbObjectError + 33, , "No encontre la hoja '" & hoja & "' en el libro de Marcas."
    End If
    ...
    If Not IsArray(v) Then Err.Raise vbObjectError + 34, , "La hoja de Marcas salio vacia."
    ...
    If cVar = 0 Then cVar = BuscaCol(v, cab, "VAR AJ|VARIACION AJ|VAR. AJ|VARAJ")
    If cDia = 0 Or cNom = 0 Or cVar = 0 Then
        Err.Raise vbObjectError + 35, , _
            "No pude ubicar las columnas de Marcas." & vbCrLf & vbCrLf & _
            "Dia: " & IIf(cDia > 0, NumALetra(cDia), "NO") & _
            "   Nombre: " & IIf(cNom > 0, NumALetra(cNom), "NO") & _
            "   Var Adj: " & IIf(cVar > 0, NumALetra(cVar), "NO") & vbCrLf & vbCrLf & _
            "Escribelas a mano en Config C50, C51 y C52."
    End If
        ...
        If EsNumero(v(i, cDia)) Then
            ...
            If mapa.Exists(cla) Then
                If EsNumero(v(i, cVar)) Then
                    ...
                    If vAdj > -1 Then
                        If Not acum.Exists(cla) Then acum(cla) = Array(0#, 0#, 0#, 0#, 0#, 0#)
                        arr = acum(cla)
                        For j = 0 To N_VENT - 1
                            If dia > bases(j) And dia <= dCorte Then
                    ...
                    End If
                End If
                If Not ultima.Exists(cla) Then ultima(cla) = 0#
                If dia > CDbl(ultima(cla)) And dia <= dCorte Then ultima(cla) = dia
            End If
        End If
    Next i
    ...
    For Each k In mapa.Keys
        ...
        If acum.Exists(k) Then
            ...
            For j = 0 To N_VENT - 1
                ...
                End If
            Next j
            wsFon.Cells(r, 10).Value = CDbl(ultima(k))
    ...
    Aviso "v2.10 - CargarMarcas terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
          mPasos & vbCrLf & _
          "Columnas: Dia " & NumALetra(cDia) & " . Nombre " & NumALetra(cNom) & _
          " . Var Adj " & NumALetra(cVar) & vbCrLf & _
          "Fondos en el puente: " & mapa.Count & vbCrLf & _
          "Marcas usadas: " & Format$(nUsadas, "#,##0") & vbCrLf & _
          "Corte: " & Format$(CDate(dCorte), "dd/mm/yyyy") & vbCrLf & _
          IIf(nSinNombre > 0, "REVISAR: " & nSinNombre & " con codigo pero sin nombre en la col B" & vbCrLf, "") & _
          IIf(nSinMarca > 0, "REVISAR: " & nSinMarca & " sin ninguna marca (mira la col J)" & vbCrLf, "") & _
          IIf(raros > 0, "REVISAR: " & raros & " marcas con |Var Adj| > 1. " & _
              "Si vienen en % y no en tanto por uno, el retorno sale disparado." & vbCrLf, "") & _
          vbCrLf & "Ahora corre Recalcular.", "CargarMarcas"
    Exit Sub
