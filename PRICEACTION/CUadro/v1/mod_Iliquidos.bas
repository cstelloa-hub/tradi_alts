...
'
' HOJAS
'   Config . Instrumentos . Mapa . BD_Pesos . BD_Precios . Fondos . Retornos . CUADRO . Log
'
' MACROS
...
Public Sub CrearConfig()
    ...
    MsgBox "Config e Instrumentos listas." & vbCrLf & vbCrLf & _
           "1. Pon las carpetas en C2 (vector) y C3 (FMS)." & vbCrLf & _
           "2. Ejecuta CargarFMS." & vbCrLf & _
           "3. Llena Nombre y Categoria en la hoja Instrumentos." & vbCrLf & _
           "4. Ejecuta CargarVectores.", vbInformation, "CrearConfig"
    Exit Sub
...
Public Sub CrearInstrumentos()
    ...
    ws.Range("J1").Value = "Nombre automatico"
...
Public Sub CargarFMS()
    ...
    Set datos = NuevoDic()
    AcumularCartera v, cMap, f1, 1, datos
    AcumularCartera v, cMap, f2, 2, datos

    Paso "Leyendo el total de cada fondo del Anexo I"
    totF1 = TotalAnexo(va, aMap, f1, codTot)
    totF2 = TotalAnexo(va, aMap, f2, codTot)
...
Private Function TotalAnexo(va As Variant, aMap As Object, fondo As String, codTot As String) As Double
    ...
    For i = 1 To UBound(va, 1)
        If NormFondo(Txt(va(i, aMap("fondo")))) = fondo Then
...
Public Sub CargarVectores()
    ...
    Dim nArch As Long, nSalt As Long, nFilas As Long, nHueco As Long
    ...
    For dFe = dDesde To dHasta
        If Not ya.Exists(CLng(dFe)) Then
        ...
        If Len(cod) > 0 Then
            If codigos.Exists(cod) Then
                ...
                If Not EsNumero(v(i, vMap("precio"))) Then
                    buf(nBuf, 10) = Flag(buf(nBuf, 10), "REVISAR: sin precio limpio")
                End If
                n = n + 1
            End If
        End If
...
Private Sub MapaFmsHoja(cMap As Object, ruta As String, dFms As Double, _
                        nIns As Long, totF1 As Double, totF2 As Double)
    ...
    r = r + 1: ws.Cells(r, 1).Value = "Instrumentos con posicion": ws.Cells(r, 2).Value = nIns
...
Private Function Flag(actual As Variant, nuevo As String) As String
    Dim a As String
    a = Txt(actual)
    If Len(a) = 0 Then
        Flag = nuevo
...
'===============================================================================
'  PARTE 2:  hoja Retornos  y  CUADRO
'===============================================================================
'  Layout del CUADRO
...
Private Function LeerInstrumentos(ByRef orden As Variant) As Object
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 And Len(cat) > 0 Then
            ...
            If Len(nom) = 0 Then nom = Trim$(Txt(ws.Cells(i, 10).Value))
            If Len(nom) = 0 Then nom = Trim$(Txt(ws.Cells(i, 5).Value))
            If Len(nom) = 0 Then nom = cod
            mon = UCase$(Trim$(Txt(ws.Cells(i, 9).Value)))
            If Len(mon) = 0 Then mon = "SIN MONEDA"
