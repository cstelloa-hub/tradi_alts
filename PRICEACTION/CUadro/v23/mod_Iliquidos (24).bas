...
' mod_Iliquidos
'
' VERSION: la de verdad esta en la constante VER, unas lineas mas abajo,
' y la saca la macro Version. Aqui no la escribo para que no se desfase.
'
...
'   - Si un instrumento no tiene precio en la fecha base de una ventana, la
'     ventana arranca en su primer precio disponible. El retorno existe pero
'     cubre menos dias; la columna Flag de Retornos dice cuantos.
'
...
'   CargarMarcas     lee el libro de Marcas y calcula el retorno de los fondos
'                    tradicionales. No depende del otro libro.
'   ArmarNombres     arma el nombre final: base + cupon + vencimiento.
'   CrearFondos      ya no hace falta; solo explica donde vive el puente.
...
'   6. Ruta de Marcas en Config C46, y CargarMarcas
...
Private Const VER As String = "2.22"
...
' BD_Pesos
Private Const PE_COLS As Long = 10
' 1 Codigo SBS  2 Nemonico  3 Emisor  4 Moneda  5 Val_total F1  6 Val_total F2
' 7 Peso F1  8 Peso F2  9 Fecha FMS  10 Vencimiento (FMS)
...
Private Const CC_CYP   As Long = 4
Private Const CC_P1    As Long = 5
Private Const CC_SEP   As Long = 7
Private Const CC_W1    As Long = 8
Private Const CC_SEP2  As Long = 14
Private Const CC_MEN   As Long = 15
Private Const CC_ANN   As Long = 16
Private Const CC_CAT   As Long = 17
Private Const CC_TIPO  As Long = 18
Private Const CC_COD   As Long = 19
Private Const CC_MRE   As Long = 20
...
Public Sub CrearConfig()
    ...
    Map ws, 46, "Libro de Marcas (ruta completa)", ""
    Map ws, 47, "Hoja de Marcas", "Hoja 3"
    Map ws, 48, "Fila de la cabecera", "3"
    Map ws, 49, "Primera fila de datos", "4"
    ...
    Map ws, 55, "Orden (separado por |)", _
        "Cuasi Soberanos|Iliquidos|Fondos Tradicionales|Coinversiones"
    ws.Range("D55").Value = "Las que no esten aqui van al final."
    ws.Range("D55").Font.Color = RGB(128, 128, 128)
    ws.Range("C55").Interior.Color = RGB(255, 242, 204)
    ws.Range("B57").Value = "PASE DE DOLARES A SOLES"
    ws.Range("B57").Font.Bold = True
    Cfg ws, 58, "Spread USD -> PEN (anual)", Rec(prev, "C58", 0.0133), _
        "Tu input. 0.0133 = 1.33%. En 0 no convierte nada."
    ws.Range("C58").Interior.Color = RGB(255, 242, 204)
    ws.Range("C58").NumberFormat = "0.00%"
    Map ws, 59, "Categorias que van siempre en PEN", "Cuasi Soberanos"
    ws.Range("D59").Value = "Aunque el instrumento este en USD. Separa con |"
    ws.Range("D59").Font.Color = RGB(128, 128, 128)
    ws.Range("C59").Interior.Color = RGB(255, 242, 204)
    Map ws, 60, "Categorias que ordenan por VINTAGE", "Fondos Tradicionales"
    ws.Range("D60").Value = "Las demas se ordenan por peso F1+F2, de mayor a menor."
    ws.Range("D60").Font.Color = RGB(128, 128, 128)
    ws.Range("C60").Interior.Color = RGB(255, 242, 204)
    ws.Range("B45").Value = "MARCAS (retorno de los fondos tradicionales)"
    ws.Range("B45").Font.Bold = True
    ws.Range("D50").Value = "Vacias = las busco por el texto de la cabecera."
    ws.Range("D50").Font.Color = RGB(128, 128, 128)
    ...
    ws.Range("C46:C52").Interior.Color = RGB(255, 242, 204)
...
Public Sub CrearInstrumentos()
    ...
    If Len(Txt(ws.Range("A1").Value)) > 0 Then
        ' La hoja ya existe: no se toca ni una fila de datos.
        ' Lo unico: las instrucciones vivian en la M, que ahora es Vintage.
        ' Se mudan a la O y se dejan L y M libres para ti.
        If InStr(1, Txt(ws.Range("M1").Value), "COLUMNAS QUE LLENAS", vbTextCompare) = 1 Then
            ws.Range("M1:M8").Copy Destination:=ws.Range("O1")
            ws.Range("M1:M8").ClearContents
            ws.Range("M1:M8").Interior.Pattern = xlNone
            ws.Columns("M").ColumnWidth = 12
            ws.Columns("O").ColumnWidth = 96
        End If
        If Len(Txt(ws.Range("L1").Value)) = 0 Then ws.Range("L1").Value = "Curr. Yield (manual)"
        If Len(Txt(ws.Range("M1").Value)) = 0 Then ws.Range("M1").Value = "Vintage"
        ws.Range("L1:M1").Font.Bold = True
        ws.Range("L1:M1").Interior.Color = RGB(255, 242, 204)
    ...
    ws.Range("L1").Value = "Curr. Yield (manual)"
    ws.Range("M1").Value = "Vintage"
    ws.Range("A1:M1").Font.Bold = True
    ws.Range("A1:M1").Interior.Color = RGB(217, 217, 217)
    ws.Range("B1:D1").Interior.Color = RGB(255, 242, 204)
    ws.Range("L1:M1").Interior.Color = RGB(255, 242, 204)

    ' Las instrucciones van en la O, lejos de las columnas que se usan.
    ws.Range("O1").Value = "COLUMNAS QUE LLENAS TU (las demas se refrescan solas)"
    ws.Range("O2").Value = "B  Nombre: manda sobre el automatico. Vacio = se usa el de la columna J."
    ws.Range("O3").Value = "   El vector a veces pone al estructurador y no al riesgo real, por eso existe esta columna."
    ws.Range("O4").Value = "C  Categoria: Cuasi Soberanos / Iliquidos / Coinversiones / Fondos Tradicionales."
    ws.Range("O5").Value = "   Sin categoria, el instrumento no entra al cuadro."
    ws.Range("O6").Value = "D  Fuera del Total: escribe SI para que salga del bloque y aparezca abajo."
    ws.Range("O7").Value = "L  Curr. Yield: solo para los fondos, en tanto por uno (0.0812 = 8.12%)."
    ws.Range("O8").Value = "M  Vintage: el ano, solo para los fondos. Ordena dentro de la categoria."
    ws.Range("O9").Value = "Esta hoja es append-only: los instrumentos nuevos se agregan al final."
    ws.Range("O1").Font.Bold = True
    ws.Range("O2:O9").Font.Color = RGB(128, 128, 128)
...
Public Sub CargarFMS()
    ...
    Restaurar calcPrev, scrPrev, evPrev
    Aviso "v" & VER & " - CargarFMS terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
           mPasos & vbCrLf & _
            ...
            If Len(cod) > 0 Then
                ...
                If Not datos.Exists(cod) Then
                    datos(cod) = Array(Txt(v(i, cMap("nemonico"))), _
                                       ...
                                       Txt(v(i, cMap("moneda"))), 0#, 0#, _
                                       FechaSegura(v(i, cMap("vencimiento"))))
                End If
                arr = datos(cod)
...
Private Function TotalAnexo(va As Variant, aMap As Object, fondo As String, codTot As String) As Double
    ...
    ' OJO: el texto del anexo trae un espacio delante. Sin el Trim$, el InStr
    ' devuelve 2 en vez de 1 y ninguna fila cuadra. La radiografia del Mapa si
    ' lo hacia, por eso encontraba los totales y esta funcion no.
    For i = 1 To UBound(va, 1)
        If MismoFondo(Txt(va(i, aMap("fondo"))), fondo) Then
            If InStr(1, Trim$(Txt(va(i, aMap("codigo")))), codTot, vbTextCompare) = 1 Then
...
Private Sub EscribirPesos(datos As Object, totF1 As Double, totF2 As Double, dFms As Double)
    ...
    cab = Array("Codigo SBS", "Nemonico", "Emisor", "Moneda", "Val_total F1", _
                "Val_total F2", "Peso F1", "Peso F2", "Fecha FMS", "Vencimiento (FMS)")
    ...
    For Each k In datos.Keys
        ...
        out(n, 9) = dFms
        If Not IsEmpty(arr(5)) Then out(n, 10) = arr(5)
    Next k
    ...
    ws.Range(ws.Cells(2, 9), ws.Cells(n + 1, 10)).NumberFormat = "dd/mm/yyyy"
...
Public Sub CargarMarcas()
    ' Lee el libro de Marcas y calcula el retorno compuesto de cada fondo
    ' tradicional, para las mismas seis ventanas del cuadro. Escribe el
    ' resultado en la hoja Fondos, columnas D a I.
    ' El puente es la columna B de Fondos: el nombre EXACTO como aparece en
    ' Marcas, al lado de su codigo SBS.
    ' No depende del otro libro: abre Marcas en solo lectura y la cierra.
    Dim wsCfg As Worksheet
    Dim wsFon As Worksheet
    ...
    Dim dFe As Double
    Dim vAdj As Double
    Dim dCorte As Double
    Dim bases() As Double
    Dim arr As Variant
    Dim k As Variant
    Dim nFilas As Long
    Dim nUsadas As Long
    Dim nSinMarca As Long
    Dim nSinNombre As Long
    ...
    Paso "Validando Config"
    Set wsCfg = ThisWorkbook.Worksheets(SH_CFG)
    ruta = Trim$(Txt(wsCfg.Range("C46").Value))
    If Len(ruta) = 0 Then
        Err.Raise vbObjectError + 30, , "Falta la ruta del libro de Marcas en Config C46."
    End If
    hoja = Trim$(Txt(wsCfg.Range("C47").Value))
    If Len(hoja) = 0 Then hoja = "3"
    cab = CLng(NumDef(wsCfg.Range("C48").Value, 1))
    fil0 = CLng(NumDef(wsCfg.Range("C49").Value, 2))

    Paso "Leyendo el puente desde Instrumentos"
    ' Marcas trae su propia columna de Codigo SBS, asi que el cruce es por
    ' codigo y no por nombre. Se buscan todos los que tengan categoria.
    If Not HojaExiste(SH_INS) Then
        Err.Raise vbObjectError + 31, , "No existe la hoja " & SH_INS & "."
    End If
    Set wsIns = ThisWorkbook.Worksheets(SH_INS)
    Set mapa = NuevoDic()
    ult = wsIns.Cells(wsIns.Rows.Count, 1).End(xlUp).Row
    For i = 2 To ult
        cod = SinGuiones(Txt(wsIns.Cells(i, 1).Value))
        If Len(cod) > 0 And Len(Trim$(Txt(wsIns.Cells(i, 3).Value))) > 0 Then
            nom = Trim$(Txt(wsIns.Cells(i, 10).Value))
            If Len(nom) = 0 Then nom = Trim$(Txt(wsIns.Cells(i, 2).Value))
    ...
    If mapa.Count = 0 Then
        Err.Raise vbObjectError + 32, , _
            "En Instrumentos no hay ninguna fila con codigo SBS y categoria."
    End If

    Paso "Abriendo Marcas en solo lectura"
    yaAbierto = LibroYaAbierto(ruta)
    Set wb = Workbooks.Open(Filename:=ruta, ReadOnly:=True, UpdateLinks:=0, AddToMru:=False)
    On Error Resume Next
    Set ws = wb.Worksheets(hoja)
    On Error GoTo Falla
    If ws Is Nothing Then
        If Not yaAbierto Then wb.Close SaveChanges:=False
        Err.Raise vbObjectError + 33, , "No encontre la hoja '" & hoja & "' en Marcas."
    End If

    Paso "Leyendo las marcas a memoria"
    ' Anclado en A1: si la hoja no empieza en A1, UsedRange corre las columnas
    ' y las letras dejan de coincidir.
    ultF = ws.UsedRange.Row + ws.UsedRange.Rows.Count - 1
    ultC = ws.UsedRange.Column + ws.UsedRange.Columns.Count - 1
    If ultF < 1 Then ultF = 1
    If ultC < 1 Then ultC = 1
    v = ws.Range(ws.Cells(1, 1), ws.Cells(ultF, ultC)).Value2
    If Not yaAbierto Then wb.Close SaveChanges:=False
    Set wb = Nothing
    Set ws = Nothing
    If Not IsArray(v) Then
        Err.Raise vbObjectError + 34, , "La hoja de Marcas salio vacia."
    End If

    Paso "Ubicando las columnas"
    colDia = Letra(Txt(wsCfg.Range("C50").Value), 0)
    colNom = Letra(Txt(wsCfg.Range("C51").Value), 0)
    colVar = Letra(Txt(wsCfg.Range("C52").Value), 0)
    filaCab = cab
    If colDia = 0 Then colDia = BuscaCol(v, cab, "DIA|FECHA", filaCab)
    If colNom = 0 Then colNom = BuscaCol(v, cab, "CODIGO SBS|CODIGO", filaCab)
    If colVar = 0 Then colVar = BuscaCol(v, cab, "VAR AJ|VARIACION AJ|VARAJ|AJUSTADA", filaCab)
    If filaCab > cab Then
        cab = filaCab
        If fil0 <= cab Then fil0 = cab + 1
    End If
    If colDia = 0 Or colNom = 0 Or colVar = 0 Then
        msg = "No pude ubicar las columnas de Marcas." & vbCrLf & vbCrLf
        msg = msg & "Dia: " & LetraODash(colDia)
        msg = msg & "   Codigo SBS: " & LetraODash(colNom)
        msg = msg & "   Var Adj: " & LetraODash(colVar) & vbCrLf & vbCrLf
        msg = msg & "Esto es lo que lei en las primeras filas de la hoja '" & hoja & "':"
        msg = msg & vbCrLf & Cabeceras(v)
        msg = msg & vbCrLf & "Escribe las letras a mano en Config C50, C51 y C52."
        Err.Raise vbObjectError + 35, , msg
    End If

    Paso "Componiendo los retornos"
    ...
    Set ultMar = NuevoDic()
    nFilas = UBound(v, 1)
    For i = fil0 To nFilas
        If EsNumero(v(i, colDia)) Then
            dFe = CDbl(v(i, colDia))
            cla = SinGuiones(Txt(v(i, colNom)))
            If mapa.Exists(cla) Then
    ...
    Paso "Escribiendo la hoja Fondos"
    Set wsFon = HojaFondos()
    fila = 1
    For Each k In mapa.Keys
        fila = fila + 1
        cod = CStr(k)
        wsFon.Cells(fila, 1).Value = "'" & cod
        wsFon.Cells(fila, 2).Value = CStr(mapa(k))
        If acum.Exists(k) Then
            arr = acum(k)
            For j = 0 To N_VENT - 1
                If CDbl(arr(j)) = 0 Then
                    wsFon.Cells(fila, 4 + j).ClearContents
                Else
                    wsFon.Cells(fila, 4 + j).Value = Exp(CDbl(arr(j))) - 1
                End If
            Next j
            wsFon.Cells(fila, 10).Value = CDbl(ultMar(k))
            wsFon.Cells(fila, 10).NumberFormat = "dd/mm/yyyy"
        Else
            ' sin marcas: es un bono, su retorno sale del vector. No se escribe.
            nSinMarca = nSinMarca + 1
            fila = fila - 1
        End If
    Next k
    wsFon.Range(wsFon.Cells(2, 4), wsFon.Cells(fila, 9)).NumberFormat = "0.00%"
    wsFon.Columns(2).ColumnWidth = 36
    wsFon.Columns(10).ColumnWidth = 40

    Restaurar calcPrev, scrPrev, evPrev

    msg = "v" & VER & " - CargarMarcas terminado en " & Format$(Timer - mT0, "0.0") & " s."
    msg = msg & vbCrLf & vbCrLf & mPasos & vbCrLf
    msg = msg & "Columnas: Dia " & NumALetra(colDia)
    msg = msg & " . Nombre " & NumALetra(colNom)
    msg = msg & " . Var Adj " & NumALetra(colVar) & vbCrLf
    msg = msg & "Instrumentos buscados en Marcas: " & mapa.Count & vbCrLf
    ...
    End If
    If raros > 0 Then
        msg = msg & "REVISAR: " & raros & " marcas con |Var Adj| mayor a 1." & vbCrLf
        msg = msg & "Si vienen en % y no en tanto por uno, el retorno sale disparado." & vbCrLf
    End If
    msg = msg & vbCrLf & "Ahora corre Recalcular."
    Aviso msg, "CargarMarcas"
    Exit Sub
...
Private Function HojaFondos() As Worksheet
    ' Deja la hoja lista para escribir. Es un RESULTADO: se rearma cada vez.
    Dim ws As Worksheet, cab As Variant
    Set ws = HojaOCrea(SH_FON)
    ws.Cells.Clear
    ws.Cells.Interior.Pattern = xlNone
    cab = Array("Codigo SBS", "Nombre en Marcas", "Curr. Yield", _
                "WTD", "5D", "MTD", "MayoTD", "YTD", "FY", "Ultima marca")
    ws.Range(ws.Cells(1, 1), ws.Cells(1, 10)).Value = cab
...
Private Function LetraODash(c As Long) As String
    If c > 0 Then
        LetraODash = NumALetra(c)
    Else
        LetraODash = "NO"
    End If
End Function
...
Private Function BuscaCol(v As Variant, cab As Long, patrones As String, _
                          ByRef filaCab As Long) As Long
    ' Busca la columna cuya cabecera contenga alguno de los patrones. Empieza
    ' por la fila que dice Config y, si no la encuentra, barre las 15 primeras.
    Dim i As Long, j As Long, t As String, p As Variant, hasta As Long
    If Not IsArray(v) Then Exit Function
    hasta = UBound(v, 1)
    If hasta > 15 Then hasta = 15
    For Each p In Split(patrones, "|")
        For i = 1 To hasta
            If i = cab Or i <= hasta Then
                For j = LBound(v, 2) To UBound(v, 2)
                    t = NormTxt(Txt(v(i, j)))
                    If Len(t) > 0 Then
                        If InStr(1, t, CStr(p), vbBinaryCompare) > 0 Then
                            BuscaCol = j
                            If i > filaCab Then filaCab = i
                            Exit Function
                        End If
                    End If
                Next j
...
Private Function Cabeceras(v As Variant) As String
    ' Vuelca las primeras filas con texto, con su letra de columna, para que
    ' se vea de una que trae la hoja.
    Dim i As Long, j As Long, t As String, linea As String, out As String
    Dim nFil As Long, hasta As Long
    If Not IsArray(v) Then Exit Function
    hasta = UBound(v, 1)
    If hasta > 12 Then hasta = 12
    For i = 1 To hasta
        linea = ""
        For j = LBound(v, 2) To UBound(v, 2)
            t = Trim$(Txt(v(i, j)))
            If Len(t) > 0 Then
                If Len(linea) < 90 Then
                    linea = linea & NumALetra(j) & "=" & Left$(t, 14) & "  "
                End If
            End If
        Next j
        If Len(linea) > 0 Then
            nFil = nFil + 1
            out = out & "fila " & i & ":  " & linea & vbCrLf
            If nFil >= 5 Then Exit For
        End If
    Next i
    Cabeceras = out
End Function
...
Private Function LibroYaAbierto(ruta As String) As Boolean
    Dim wb As Workbook, nom As String
    nom = Mid$(ruta, InStrRev(ruta, Application.PathSeparator) + 1)
...
Public Sub ArmarNombres()
    ...
    Dim datos As Object, arr As Variant, vencFms As Object
    Dim nArm As Long, nSin As Long, nMio As Long, cup As Double, ven As Double
    ...
    ' vencimiento de respaldo: el que trae el FMS
    Set vencFms = NuevoDic()
    If HojaExiste(SH_PES) Then
        Set wsP = ThisWorkbook.Worksheets(SH_PES)
        ult = wsP.Cells(wsP.Rows.Count, 1).End(xlUp).Row
        For i = 2 To ult
            cod = SinGuiones(Txt(wsP.Cells(i, 1).Value))
            If Len(cod) > 0 And Num(wsP.Cells(i, 10).Value) >= 10000 Then
                vencFms(cod) = Num(wsP.Cells(i, 10).Value)
            End If
        Next i
    End If
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 Then
            ...
            If datos.Exists(cod) Then
                ...
                ven = Num(arr(2))
                If ven < 10000 Then
                    If vencFms.Exists(cod) Then ven = CDbl(vencFms(cod))
                End If
                ' celda vacia, no 00/01/1900
                If cup > 0 Then
                    ws.Cells(i, 7).Value = cup
                Else
                    ws.Cells(i, 7).ClearContents
                End If
                If ven >= 10000 Then
                    ws.Cells(i, 8).Value = ven
                Else
                    ws.Cells(i, 8).ClearContents
                End If
                ws.Cells(i, 10).Value = ArmaNombre(base, cup, ven)
                ws.Cells(i, 11).Value = "SI"
                If cup > 0 Or ven >= 10000 Then
                    nArm = nArm + 1
                Else
                    nMio = nMio + 1
                End If
...
Public Sub Version()
    MsgBox "mod_Iliquidos  v" & VER, vbInformation, "Version"
End Sub
...
Public Sub Diagnostico()
    ...
    On Error GoTo Falla
    m = "DIAGNOSTICO  -  mod_Iliquidos v" & VER & vbCrLf & String$(34, "-") & vbCrLf
...
Public Sub CargarVectores()
    ...
    t = Timer - mT0
    msg = "v" & VER & " - CargarVectores terminado en " & Format$(t, "0.0") & " s"
    If t > 90 Then msg = msg & " (" & Format$(t / 60, "0.0") & " min)"
    msg = msg & "." & vbCrLf & vbCrLf & mPasos & vbCrLf
    msg = msg & "Archivos leidos ahora: " & nArch & vbCrLf
    msg = msg & "Fechas que ya estaban en BD_Precios: " & nYa & vbCrLf
    msg = msg & "Fechas sin archivo (fines de semana y feriados): " & nSalt & vbCrLf
    msg = msg & "Filas agregadas: " & Format$(nFilas, "#,##0") & vbCrLf
    msg = msg & "Codigos seguidos: " & codigos.Count & vbCrLf
    msg = msg & "BD_Precios queda con " & FilasPrecios() & " filas en total." & vbCrLf & vbCrLf
    If nArch = 0 Then
        If nYa > 0 Then
            msg = msg & "No leyo ningun archivo porque esas fechas YA estaban cargadas." & vbCrLf
            msg = msg & "Para releerlas, borra esas filas de BD_Precios." & vbCrLf & vbCrLf
        ElseIf nSalt > 0 Then
            msg = msg & "No encontre ningun archivo en la carpeta para esas fechas." & vbCrLf
            msg = msg & "Revisa Config C2 y que el archivo se llame 'AAAAMMDD RFL.xls'." & vbCrLf & vbCrLf
        End If
    End If
    msg = msg & "Revisa la hoja Mapa y la columna Flag de BD_Precios."
    Aviso msg, "CargarVectores"
    Exit Sub
...
Public Sub ActualizarTodo()
    ...
    CargarVectores
    If mFallo Then GoTo Salir
    If HojaExiste(SH_FON) Then
        If Len(Trim$(Txt(ThisWorkbook.Worksheets(SH_CFG).Range("C46").Value))) > 0 Then
            CargarMarcas
            If mFallo Then GoTo Salir
        End If
    End If
...
Private Function RadiografiaAnexo(va As Variant, aMap As Object, codTot As String, _
    ...
    Dim fo As String, linea As String, listaFo As String, muestras As String
    ...
    For i = 1 To n
        ...
        If Len(fo) > 0 And Len(fo) <= 4 Then
            If Not vistos.Exists(fo) Then
                vistos(fo) = 1
                If Len(listaFo) < 60 Then listaFo = listaFo & IIf(Len(listaFo) > 0, " . ", "") & fo
            End If
        End If
...
Private Function EsNumero(v As Variant) As Boolean
    ...
    ElseIf VarType(v) = vbDate Then
        ' OJO: IsNumeric() de una fecha devuelve False. Por eso el vencimiento
        ' salia 0 y se veia como 00/01/1900. Una fecha SI es un numero.
        EsNumero = True
...
Public Sub Recalcular()
    ...
    Dim nsin As Long, ndup As Long, avisos As String
    Dim scrPrev As Boolean, calcPrev As XlCalculation, evPrev As Boolean
    ...
    Set ins = LeerInstrumentos(orden, nsin, ndup, avisos)
    ...
    DefinirNombre "cfgSpread", SH_CFG, "$C$58"
    ...
    Aviso "v" & VER & " - Recalcular terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
           mPasos & vbCrLf & _
           "Instrumentos en el cuadro: " & ins.Count & vbCrLf & _
           IIf(nsin > 0, "Filas SIN categoria (no entran): " & nsin & vbCrLf, "") & _
           IIf(ndup > 0, "Codigos repetidos (solo entro el primero): " & ndup & vbCrLf, "") & _
           IIf(Len(avisos) > 0, vbCrLf & avisos, "") & _
...
Private Function MonedaBloque(cat As String, mon As String) As String
    ' Hay categorias que van siempre al bloque PEN, aunque el instrumento este
    ' en dolares: en el cuadro se muestran ya pasadas a soles. La moneda real
    ' se guarda aparte, porque es la que decide si se le suma el spread.
    Dim cfgPen As String, p As Variant
    MonedaBloque = mon
    On Error Resume Next
    cfgPen = Txt(ThisWorkbook.Worksheets(SH_CFG).Range("C59").Value)
    On Error GoTo 0
    If Len(Trim$(cfgPen)) = 0 Then Exit Function
    For Each p In Split(cfgPen, "|")
        If Len(Trim$(CStr(p))) > 0 Then
            If NormTxt(cat) = NormTxt(CStr(p)) Then
                MonedaBloque = "PEN"
                Exit Function
            End If
        End If
...
Private Function LeerInstrumentos(ByRef orden As Variant, ByRef nsin As Long, _
                                  ByRef ndup As Long, ByRef avisos As String) As Object
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 And Len(cat) = 0 Then
            nsin = nsin + 1
            If Len(avisos) < 200 Then
                avisos = avisos & "  fila " & i & ": " & _
                         Left$(Trim$(Txt(ws.Cells(i, 10).Value)) & _
                               Trim$(Txt(ws.Cells(i, 5).Value)), 30) & " (sin categoria)" & vbCrLf
            End If
        End If
        If Len(cod) > 0 And Len(cat) > 0 Then
            ...
            If Not d.Exists(cod) Then
                d(cod) = Array(cod, nom, cat, MonedaBloque(cat, mon), _
                               UCase$(Left$(Trim$(Txt(ws.Cells(i, 4).Value)) & "  ", 2)) = "SI", _
                               ws.Cells(i, 12).Value, Num(ws.Cells(i, 13).Value), mon)
                n = n + 1
                lst(n) = cod
            Else
                ' mismo codigo SBS dos veces: solo entra el primero
                ndup = ndup + 1
                If Len(avisos) < 200 Then
                    avisos = avisos & "  fila " & i & ": " & Left$(nom, 30) & _
                             " (codigo repetido)" & vbCrLf
                End If
            End If
        End If
    Next i
...
Public Sub CrearFondos()
    ' La hoja Fondos ya no se llena a mano: la escribe CargarMarcas.
    MsgBox "Ya no hace falta llenar la hoja " & SH_FON & "." & vbCrLf & vbCrLf & _
           "El puente vive en la hoja Instrumentos:" & vbCrLf & _
           "  - se buscan en Marcas los que NO estan en el vector (columna K)" & vbCrLf & _
           "  - el nombre que se busca es el de la columna B" & vbCrLf & _
           "  - si en Marcas se llama distinto, escribelo en la columna L" & vbCrLf & vbCrLf & _
           "Pon la ruta de Marcas en Config C46 y corre CargarMarcas.", _
           vbInformation, "Fondos"
End Sub


Private Function LeerFondos() As Object
    ' codigo SBS -> Array(curr yield, WTD, 5D, MTD, MayoTD, YTD, FY)
    Dim ws As Worksheet, d As Object, i As Long, ult As Long, j As Long
    Dim cod As String, v(0 To 6) As Variant, hay As Boolean
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 Then
            hay = False
            For j = 0 To 6
                If EsNumero(ws.Cells(i, j + 3).Value) Then
                    v(j) = CDbl(ws.Cells(i, j + 3).Value)
                    If j > 0 Then hay = True
                Else
                    v(j) = Empty
                End If
            Next j
            If hay Then
                d(cod) = Array(v(0), v(1), v(2), v(3), v(4), v(5), v(6))
            End If
        End If
    Next i
End Function
    ...
    If IsArray(orden) Then
        For i = LBound(orden) To UBound(orden)
            ...
            For j = 0 To N_VENT - 1
                ...
                ' Si no hay precio en la fecha base, se arranca desde el primer
                ' precio que exista dentro de la ventana. La columna Flag lo avisa.
                ws.Cells(r, 9).Formula = _
                    "=IFERROR(IF(MAXIFS(prFecha,prCod,$A" & r & ",prFecha,""<=""&G" & r & ")>0," & _
                    "MAXIFS(prFecha,prCod,$A" & r & ",prFecha,""<=""&G" & r & ")," & _
                    "IF(MINIFS(prFecha,prCod,$A" & r & ",prFecha,""<=""&H" & r & ")>0," & _
                    "MINIFS(prFecha,prCod,$A" & r & ",prFecha,""<=""&H" & r & "),"""")),"""")"
                ...
                ws.Cells(r, 19).Formula = _
                    ...
                    "IF(I" & r & ">G" & r & ",""REVISAR: sin precio al ""&TEXT(G" & r & _
                    ",""dd/mm/yy"")&""; la ventana arranca el ""&TEXT(I" & r & ",""dd/mm/yy"")&" & _
                    """ (""&M" & r & "&"" dias en vez de ""&(H" & r & "-G" & r & ")&"")""," & _
...
Private Sub ArmarCuadro(ins As Object, orden As Variant, dCorte As Double)
    ...
    Dim fondos As Object
    Dim lista As Variant
    Dim pesos As Object

    Set fondos = LeerFondos()
    Set pesos = PesosPorCodigo()
    vent = Array("WTD", "5D", "MTD", "MayoTD", "YTD", "FY")
    ...
    ws.Range("B3").Value = "Total return = variacion de precio limpio + cupon devengado (dias/365)" & _
        "   |   peso = Val_total / total del fondo (FMS)   |   filas grises: promedio ponderado F1+F2" & _
        "   |   lo que esta en USD pasa a soles sumando el spread de Config C58, prorrateado por dias"
    ...
    Next j
    ws.Cells(CU_CAB, CC_CY).Value = "CY"
    ws.Cells(CU_CAB, CC_CYP).Value = "CY_PEN"
    ...
    ' --- filas ---
    Set grupos = NuevoDic()
    monedas = OrdenMonedas(ins, orden)
    r1 = CU_DAT
    r = CU_DAT - 1
    ...
    For im = LBound(monedas) To UBound(monedas)
        ...
        For ic = LBound(cats) To UBound(cats)
            ...
            lista = OrdenFilas(ins, orden, mon, cat, pesos)
            If IsArray(lista) Then
                If UBound(lista) >= LBound(lista) Then
                    For i = LBound(lista) To UBound(lista)
                        arr = ins(lista(i))
                        r = r + 1
                        FilaInstrumento ws, r, mon, cat, CStr(arr(0)), CStr(arr(1)), _
                                        "INS", fondos, arr(5), CStr(arr(7))
                    Next i
                End If
            End If
        Next ic
    ...
    If hayFuera Then
        ...
        For i = LBound(orden) To UBound(orden)
            ...
            If arr(4) Then
                r = r + 1
                FilaInstrumento ws, r, "", "", CStr(arr(0)), CStr(arr(1)), "OUT", fondos, _
                                arr(5), CStr(arr(7))
            End If
        Next i
    End If
...
Private Sub FilaInstrumento(ws As Worksheet, r As Long, mon As String, cat As String, _
                            cod As String, nom As String, tipo As String, fondos As Object, _
                            cyMan As Variant, monReal As String)
    Dim j As Long, cl As String, colYTD As String
    Dim esFondo As Boolean, f As Variant
    Dim cCod As String, cMon As String

    If Not fondos Is Nothing Then esFondo = fondos.Exists(cod)
    If esFondo Then f = fondos(cod)
    ...
    ws.Cells(r, CC_MRE).Value = monReal
    If tipo = "INS" Then ws.Cells(r, CC_NOM).IndentLevel = 2

    cCod = "$" & NumALetra(CC_COD) & r
    ' OJO: el spread mira la moneda REAL, no la del bloque donde se muestra.
    cMon = "$" & NumALetra(CC_MRE) & r

    ' el peso SIEMPRE sale del FMS, sea bono o fondo
    ws.Cells(r, CC_P1).Formula = "=IFERROR(SUMIFS(pePesoF1,peCod," & cCod & "),0)"
    ws.Cells(r, CC_P1 + 1).Formula = "=IFERROR(SUMIFS(pePesoF2,peCod," & cCod & "),0)"

    If esFondo Then
        ' fondo tradicional: los retornos vienen de la hoja Fondos (Marcas)
        If Not IsEmpty(f(0)) Then ws.Cells(r, CC_CY).Value = f(0)
        For j = 0 To N_VENT - 1
            If Not IsEmpty(f(1 + j)) Then
                cl = NumALetra(CC_W1 + j)
                ws.Cells(r, CC_W1 + j).Formula = _
                    "=" & Format$(CDbl(f(1 + j)), "0.00000000") & "+" & Pase(cMon, cl)
            End If
        Next j
    Else
        ws.Cells(r, CC_CY).Formula = _
            "=IFERROR(AVERAGEIFS(reCY,reCod," & cCod & ",reVent,""YTD""),"""")"
        For j = 0 To N_VENT - 1
            cl = NumALetra(CC_W1 + j)
            ws.Cells(r, CC_W1 + j).Formula = _
                "=IFERROR(AVERAGEIFS(reSuma,reCod," & cCod & ",reVent," & cl & "$" & CU_CAB & ")+" & _
                Pase(cMon, cl) & ","""")"
        Next j
    End If

    ' El Curr. Yield de la columna L de Instrumentos manda sobre lo calculado.
    ' Para un fondo no hay forma de calcularlo: ese es su unico origen.
    If EsNumero(cyMan) Then ws.Cells(r, CC_CY).Value = CDbl(cyMan)

    ' CY_PEN: el mismo yield mas el spread, si el instrumento esta en dolares.
    ws.Cells(r, CC_CYP).Formula = _
        "=IF(NOT(ISNUMBER(" & NumALetra(CC_CY) & r & ")),""""," & _
        NumALetra(CC_CY) & r & "+IF(" & cMon & "=""USD"",cfgSpread,0))"

    ' Vacio, no NA(): un #N/D en estas columnas envenena el SUMPRODUCT de las
    ' filas grises y deja el bloque entero en blanco.
    colYTD = NumALetra(CC_W1 + 4)
    ws.Cells(r, CC_ANN).Formula = _
        "=IF(NOT(ISNUMBER(" & colYTD & r & ")),""""," & _
        "IFERROR((1+" & colYTD & r & ")^(365/(" & colYTD & "$" & CU_FIN & "-" & _
        colYTD & "$" & CU_BASE & "))-1,""""))"
    ws.Cells(r, CC_MEN).Formula = _
        "=IF(NOT(ISNUMBER(" & NumALetra(CC_ANN) & r & ")),""""," & _
        NumALetra(CC_ANN) & r & "/12)"
End Sub
...
Private Sub FilaGrupo(ws As Worksheet, r As Long, tipo As String, r1 As Long, r2 As Long)
    ...
    ws.Cells(r, CC_CYP).Formula = "=" & Prom(w, cond, CC_CYP, r1, r2)
    For j = 0 To N_VENT - 1
...
Private Function Pase(cMon As String, cl As String) As String
    ' Spread anual prorrateado por los dias de la ventana, solo si es USD.
    Pase = "IF(" & cMon & "=""USD"",cfgSpread*(" & cl & "$" & CU_FIN & "-" & _
           cl & "$" & CU_BASE & ")/365,0)"
End Function
...
Private Function OrdenMonedas(ins As Object, orden As Variant) As Variant
...
Private Function OrdenCategorias(ins As Object, orden As Variant, mon As String) As Variant
    ' Sale en el orden de Config C55 (por riesgo). Lo que no este ahi, al final.
    Dim d As Object, i As Long, arr As Variant, k As Variant
    Dim res() As String, n As Long, p As Variant, ordCfg As String
    Set d = NuevoDic()
    For i = LBound(orden) To UBound(orden)
        arr = ins(orden(i))
    ...
    ReDim res(0 To d.Count - 1)
    n = -1
    ordCfg = Txt(ThisWorkbook.Worksheets(SH_CFG).Range("C55").Value)
    If Len(Trim$(ordCfg)) > 0 Then
        For Each p In Split(ordCfg, "|")
            For Each k In d.Keys
                If CLng(d(k)) = 1 And NormTxt(CStr(k)) = NormTxt(CStr(p)) Then
                    n = n + 1
                    res(n) = CStr(k)
                    d(k) = 2
                End If
...
Private Function OrdenFilas(ins As Object, orden As Variant, mon As String, _
                            cat As String, pesos As Object) As Variant
    ' Dentro de una categoria:
    '   - si esta en Config C60, se ordena por VINTAGE, de mas antiguo a mas nuevo
    '   - si no, por PESO F1+F2, de mayor a menor
    ' Los que no tienen el dato se van al final, en el orden de la hoja.
    Dim i As Long, n As Long, j As Long, k As Long
    Dim arr As Variant, res() As String, cri() As Double
    Dim t As String, tv As Double, porVin As Boolean

    porVin = CategoriaPorVintage(cat)
    n = -1
    ReDim res(0 To UBound(orden) - LBound(orden) + 1)
    ReDim cri(0 To UBound(orden) - LBound(orden) + 1)
    For i = LBound(orden) To UBound(orden)
        arr = ins(orden(i))
        If CStr(arr(3)) = mon And CStr(arr(2)) = cat And Not arr(4) Then
            n = n + 1
            res(n) = CStr(arr(0))
            If porVin Then
                cri(n) = Num(arr(6))
                If cri(n) <= 0 Then cri(n) = 99999
            Else
                ' peso al reves, para que el mayor quede primero con el mismo orden
                cri(n) = -PesoDe(pesos, CStr(arr(0)))
            End If
        End If
    Next i
    If n < 0 Then
    ...
    ReDim Preserve cri(0 To n)
    For j = 0 To n - 1
        For k = j + 1 To n
            If cri(k) < cri(j) Then
                tv = cri(j): cri(j) = cri(k): cri(k) = tv
                t = res(j): res(j) = res(k): res(k) = t
            End If
        Next k
    Next j
    OrdenFilas = res
...
Private Function CategoriaPorVintage(cat As String) As Boolean
    Dim cfgVin As String, p As Variant
    On Error Resume Next
    cfgVin = Txt(ThisWorkbook.Worksheets(SH_CFG).Range("C60").Value)
    On Error GoTo 0
    If Len(Trim$(cfgVin)) = 0 Then Exit Function
    For Each p In Split(cfgVin, "|")
        If Len(Trim$(CStr(p))) > 0 Then
            If NormTxt(cat) = NormTxt(CStr(p)) Then
                CategoriaPorVintage = True
                Exit Function
            End If
        End If
    Next p
...
Private Function PesoDe(pesos As Object, cod As String) As Double
    If pesos Is Nothing Then Exit Function
    If pesos.Exists(cod) Then PesoDe = CDbl(pesos(cod))
End Function


Private Function PesosPorCodigo() As Object
    ' codigo -> peso F1 + peso F2, leido de BD_Pesos
    Dim d As Object, ws As Worksheet, i As Long, ult As Long, cod As String
    Set d = NuevoDic()
    Set PesosPorCodigo = d
    If Not HojaExiste(SH_PES) Then Exit Function
    Set ws = ThisWorkbook.Worksheets(SH_PES)
    ult = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    For i = 2 To ult
        cod = SinGuiones(Txt(ws.Cells(i, 1).Value))
        If Len(cod) > 0 Then
            d(cod) = Num(ws.Cells(i, 7).Value) + Num(ws.Cells(i, 8).Value)
        End If
    Next i
End Function
...
Private Sub FormatearCuadro(ws As Worksheet, ultFila As Long)
    ...
    ws.Range(ws.Columns(CC_CY), ws.Columns(CC_CYP)).ColumnWidth = 10
    ws.Range(ws.Columns(CC_P1), ws.Columns(CC_P1 + 1)).ColumnWidth = 10
