...
Private Function CensoAssets(v As Variant, cMap As Object, f1 As String, f2 As String, _
    ...
    For i = 1 To UBound(v, 1)
        fo = NormFondo(Txt(v(i, cMap("fondo"))))
        If fo = f1 Or fo = f2 Then
...
Public Sub CargarFMS()
    ...
    MapaFmsHoja cMap, ruta, dFms, datos.Count, totF1, totF2, censo, filCod, filAsset, nSueltos
...
Public Sub ArmarNombres()
    ...
    ' el nemonico. Si en B ya escribiste una fecha (hay una barra), se respeta
    ' tal cual y no se le agrega nada.
    ...
    For i = 2 To ult
        ...
        If Len(cod) > 0 Then
            ...
            If InStr(base, "/") > 0 Then
                ' ya escribiste el nombre completo: no se toca
                ws.Cells(i, 10).Value = base
                nMio = nMio + 1
            ElseIf datos.Exists(cod) Then
                arr = datos(cod)
                ws.Cells(i, 7).Value = Num(arr(1))
                ws.Cells(i, 8).Value = Num(arr(2))
                ws.Cells(i, 10).Value = base & " " & Cupon(Num(arr(1))) & " " & Vence(Num(arr(2)))
    ...
    MsgBox "Nombres armados." & vbCrLf & vbCrLf & _
           "Con cupon y vencimiento del vector: " & nArm & vbCrLf & _
           IIf(nMio > 0, "Escritos completos por ti (tienen fecha): " & nMio & vbCrLf, "") & _
...
Private Function Cupon(t As Double) As String
    If t <= 0 Then
        Cupon = "s/c"
    Else
        Cupon = Format$(t, "0.00")
...
Private Function Vence(d As Double) As String
    If d < 10000 Then
        Vence = "s/v"
    Else
        Vence = Format$(CDate(d), "dd/mm/yy")
    End If
End Function
