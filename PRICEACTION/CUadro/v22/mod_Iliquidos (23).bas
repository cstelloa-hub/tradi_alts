...
Private Const VER As String = "2.21"
...
Private Sub ArmarCuadro(ins As Object, orden As Variant, dCorte As Double)
    ...
    For im = LBound(monedas) To UBound(monedas)
        ...
        For ic = LBound(cats) To UBound(cats)
            ...
            lista = OrdenPorVintage(ins, orden, mon, cat)
...
Private Function OrdenPorVintage(ins As Object, orden As Variant, _
                                 mon As String, cat As String) As Variant
    ' Dentro de la categoria: primero los que tienen vintage, de menor a mayor;
    ' despues los que no lo tienen, en el orden de la hoja Instrumentos.
    Dim i As Long, n As Long, j As Long, k As Long
    Dim arr As Variant, res() As String, vin() As Double, t As String, tv As Double

    n = -1
    ReDim res(0 To UBound(orden) - LBound(orden) + 1)
    ReDim vin(0 To UBound(orden) - LBound(orden) + 1)
    For i = LBound(orden) To UBound(orden)
        arr = ins(orden(i))
        If CStr(arr(3)) = mon And CStr(arr(2)) = cat And Not arr(4) Then
            ...
            res(n) = CStr(arr(0))
            vin(n) = Num(arr(6))
            If vin(n) <= 0 Then vin(n) = 99999
        End If
    Next i
    If n < 0 Then
        OrdenPorVintage = Array()
        Exit Function
    End If
    ReDim Preserve res(0 To n)
    ReDim Preserve vin(0 To n)
    For j = 0 To n - 1
        For k = j + 1 To n
            If vin(k) < vin(j) Then
                tv = vin(j): vin(j) = vin(k): vin(k) = tv
