...
'===============================================================================
' mod_RetornoFondos  -  v1.6  -  21/09/2026
'
...
Public Sub ActualizarTodo()
    ...
    MsgBox "v1.6 - ActualizarTodo terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
           sNotaFuente & _
           mPasos & vbCrLf & _
...
Falla:
    ...
    Application.StatusBar = False
    MsgBox "v1.6 - ActualizarTodo se detuvo." & vbCrLf & vbCrLf & msg & vbCrLf & vbCrLf & _
           "Ultimo punto alcanzado:" & vbCrLf & "  >> " & mDetalle & vbCrLf & vbCrLf & _
           "Paso a paso:" & vbCrLf & mPasos, vbCritical, "ActualizarTodo"
End Sub
...
Falla:
    ...
    Application.EnableEvents = True
    Application.StatusBar = False
    MsgBox "Recalcular se detuvo." & vbCrLf & vbCrLf & Err.Description & vbCrLf & vbCrLf & _
           "Ultimo punto alcanzado:" & vbCrLf & "  >> " & mDetalle, vbCritical, "Recalcular"
End Sub
...
Private Sub ArmarCuadro(libro As String)
    ...
    D "ArmarCuadro: abriendo Config y BD"
    Set wsCfg = ThisWorkbook.Worksheets(SH_CFG)
    ...
    ' --- corte ---
    D "ArmarCuadro: determinando la fecha de corte"
    ...
    D "ArmarCuadro: calculando las bases de las ventanas"
    dBaseMin = BaseMasAntigua(wsCfg, dCorte)

    D "ArmarCuadro: leyendo la hoja Clasif"
    Set cla = LeerClasif(claOrden)

    D "ArmarCuadro: armando la estructura de filas"
    Set bloques = ArmarEstructura(wsBD, ultBD, libro, dCorte, dBaseMin, claOrden, nExcl)

    ' --- hoja ---
    D "ArmarCuadro: creando y limpiando la hoja CUADRO"
    ...
    D "ArmarCuadro: encabezado de ventanas"
    EscribirEncabezadoVentanas ws

    ' --- filas ---
    D "ArmarCuadro: escribiendo las filas"
    EscribirFilas ws, bloques, libro

    D "ArmarCuadro: aplicando formato"
...
Private Sub EscribirFilas(ws As Worksheet, bloques As Object, libro As String)
    ...
    For Each kB In bloques.Keys
        r = r + 1
        filaB = r
        D "Filas: bloque '" & TextoSeguro(kB) & "' en la fila " & r
        tipos(r) = "BLQ"
        ...
        For Each kS In subs.Keys
            ...
            If TextoSeguro(kS) <> "" Then
                r = r + 1
                filaS = r
                D "Filas: subgrupo '" & TextoSeguro(kS) & "' en la fila " & r
                tipos(r) = "SUB"
                ws.Cells(r, CU_COL_BLQ).Value = TextoSeguro(kB)
            ...
            For Each kF In fondos.Keys
                r = r + 1
                D "Filas: fondo '" & TextoSeguro(kF) & "' en la fila " & r
                tipos(r) = "FND"
                ws.Cells(r, CU_COL_BLQ).Value = TextoSeguro(kB)
                ws.Cells(r, CU_COL_SUB).Value = TextoSeguro(kS)
                ...
                ' pesos
                D "Filas: formulas de peso, fila " & r
                For j = 0 To 2
                    c = CU_COL_P1 + j
                    ws.Cells(r, c).Formula = FormulaPeso(r, j + 1)
                Next j
                ' retornos
                D "Filas: formulas de retorno, fila " & r
                For j = 0 To N_VENT - 1
                    c = CU_COL_W1 + j
            ...
            If filaS > 0 Then
                D "Filas: suma del subgrupo en la fila " & filaS
        ...
        D "Filas: suma del bloque en la fila " & filaB
        For j = 0 To 2
            c = CU_COL_P1 + j
    ...
    D "Filas: guardando los tipos de fila"
...
Private Function ArmarEstructura(wsBD As Worksheet, ultBD As Long, libro As String, _
    ...
    Dim ordenBloques As Variant, ordenSubs As Variant, nombres As Variant
    Dim ib As Long, isb As Long, inm As Long

    D "Estructura: leyendo la BD a memoria"
    ...
    D "Estructura: filtro de posicion"
    ...
    D "Estructura: ordenando los bloques"
    ordenBloques = OrdenBloques(v, libro)

    For ib = LBound(ordenBloques) To UBound(ordenBloques)
        sB = TextoSeguro(ordenBloques(ib))
        D "Estructura: bloque '" & sB & "'"
        Set subs = CreateObject("Scripting.Dictionary"): subs.CompareMode = 1

        ordenSubs = OrdenSubs(v, sB, libro, claOrden)

        For isb = LBound(ordenSubs) To UBound(ordenSubs)
            sS = TextoSeguro(ordenSubs(isb))
            D "Estructura: bloque '" & sB & "' subgrupo '" & sS & "'"
            ...
            D "Estructura: guardando subgrupo '" & sS & "' (" & fondos.Count & " fondos)"
            If fondos.Count > 0 Then Set subs(sS) = fondos
        Next isb

        D "Estructura: guardando bloque '" & sB & "' (" & subs.Count & " subgrupos)"
...
Private Function PareceReferencia(nombre As String) As Boolean
...
Private Function LetraANum(letra As String) As Long
