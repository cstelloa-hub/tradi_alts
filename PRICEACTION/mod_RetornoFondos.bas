...
'===============================================================================
' mod_RetornoFondos  -  v1.0  -  21/09/2026
'
...
Public Sub ActualizarTodo()
    ...
    MsgBox "ActualizarTodo terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
           mPasos & vbCrLf & _
...
Falla:
    ...
    Application.StatusBar = False
    MsgBox "ActualizarTodo se detuvo." & vbCrLf & vbCrLf & msg & vbCrLf & vbCrLf & _
...
Falla:
    ...
    Application.EnableEvents = True
    Application.StatusBar = False
    MsgBox "Recalcular se detuvo." & vbCrLf & vbCrLf & Err.Description, vbCritical, "Recalcular"
End Sub
...
Siguiente:
    ...
    DefinirNombre "bdP1", SH_BD, "$N$2:$N$" & ultBD
    DefinirNombre "bdP2", SH_BD, "$O$2:$O$" & ultBD
    DefinirNombre "bdP3", SH_BD, "$P$2:$P$" & ultBD
...
Private Sub EscribirFilas(ws As Worksheet, bloques As Object, libro As String)
    ...
    For Each kB In bloques.Keys
        ...
        filaB = r
        tipos(r) = "BLQ"
        ws.Cells(r, CU_COL_BLQ).Value = CStr(kB)
        ws.Cells(r, CU_COL_NOM).Value = CStr(kB)
        ...
        For Each kS In subs.Keys
            ...
            If CStr(kS) <> "" Then
                ...
                tipos(r) = "SUB"
                ws.Cells(r, CU_COL_BLQ).Value = CStr(kB)
                ws.Cells(r, CU_COL_SUB).Value = CStr(kS)
                ws.Cells(r, CU_COL_NOM).Value = CStr(kS)
            Else
                filaS = 0
            End If
            ...
            For Each kF In fondos.Keys
                ...
                ws.Cells(r, CU_COL_BLQ).Value = CStr(kB)
                ws.Cells(r, CU_COL_SUB).Value = CStr(kS)
                ws.Cells(r, CU_COL_NOM).Value = CStr(kF)
...
Private Function FormulaSumaGrupo(c As Long, filas As Object) As String
    Dim s As String, k As Variant, col As String
    col = NumALetra(c)
    s = ""
    ...
    If Len(s) = 0 Then
        FormulaSumaGrupo = ""
    Else
        FormulaSumaGrupo = "=IF(SUM(" & s & ")=0,"""",SUM(" & s & "))"
    End If
End Function
...
Private Function ArmarEstructura(wsBD As Worksheet, ultBD As Long, libro As String, _
    ...
    For i = 1 To UBound(v, 1)
        If IsNumeric(v(i, 1)) Then
            ...
            sN = CStr(v(i, 4))
            If Len(sN) > 0 Then
                If Not vistos.Exists(sN) Then vistos(sN) = CStr(v(i, 6)) & "|" & CStr(v(i, 7))
    ...
    For ib = LBound(ordenBloques) To UBound(ordenBloques)
        ...
        For isb = LBound(ordenSubs) To UBound(ordenSubs)
            ...
            If fondos.Count > 0 Then subs(sS) = fondos
        Next isb

        If subs.Count > 0 Then bloques(sB) = subs
    Next ib
...
Private Function OrdenSubs(v As Variant, bloque As String, libro As String, _
    ...
    Set d = CreateObject("Scripting.Dictionary"): d.CompareMode = 1
    For i = 1 To UBound(v, 1)
        If StrComp(Trim$(CStr(v(i, 6))), bloque, vbTextCompare) = 0 Then
            s = Trim$(CStr(v(i, 7)))
            d(s) = 1
        End If
    Next i
...
Private Function NombresDe(v As Variant, bloque As String, subg As String) As Variant
    ...
    For i = 1 To UBound(v, 1)
        If StrComp(Trim$(CStr(v(i, 6))), bloque, vbTextCompare) = 0 Then
            If StrComp(Trim$(CStr(v(i, 7))), subg, vbTextCompare) = 0 Then
                s = Trim$(CStr(v(i, 4)))
                If Len(s) > 0 Then d(s) = 1
            End If
        End If
...
Private Sub FormatearCuadro(ws As Worksheet, libro As String)
    ...
    ws.Range(ws.Columns(CU_COL_P1), ws.Columns(CU_COL_P1 + 2)).ColumnWidth = 8
    ws.Columns(CU_COL_SEP).ColumnWidth = 1.6
    ws.Range(ws.Columns(CU_COL_W1), ws.Columns(CU_COL_W1 + N_VENT - 1)).ColumnWidth = 10
    ...
    For r = CU_FILA_DAT To ultFila
        tipo = CStr(ws.Cells(r, CU_COL_SUB + 1).Value)
        Select Case tipo
            Case "BLQ"
...
Private Sub DefinirNombre(nombre As String, hoja As String, refLocal As String)
    ...
    On Error GoTo 0
    ThisWorkbook.Names.Add Name:=nombre, RefersTo:="='" & hoja & "'!" & refLocal
End Sub
