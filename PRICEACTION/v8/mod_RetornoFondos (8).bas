...
'===============================================================================
' mod_RetornoFondos  -  v3.2  -  21/09/2026
'
...
Private Const SH_MON As String = "Monedas"
Private Const SH_DAT As String = "Datos"

'--- layout del CUADRO --------------------------------------------------------
'  A oculta bloque | B nombre | C vintage | D gross yield | E F1 | F F2 | G sep
'  H..M ventanas   | N sep    | O Mensual YTD | P Ann YTD
'  Q Ult. marca (fecha) | R Ret. ult. marca | S oculta subgrupo
'  T oculta tipo de fila (BLQ / SUB / FND / TOT / OUT)
Private Const CU_FILA_FIN   As Long = 4
Private Const CU_FILA_BASE  As Long = 5
Private Const CU_FILA_CAB   As Long = 6
Private Const CU_FILA_DAT   As Long = 7
Private Const CU_COL_BLQ    As Long = 1
Private Const CU_COL_NOM    As Long = 2
Private Const CU_COL_VIN    As Long = 3
Private Const CU_COL_GY     As Long = 4
Private Const CU_COL_P1     As Long = 5
Private Const N_VENT        As Long = 6

' El ancho del cuadro depende del libro: TRAD lleva F1 y F2, ALT lleva ademas F3.
' Todo lo que va a la derecha de los pesos se corre, asi que son variables y no
' constantes. FijarLayout las calcula a partir de Config!C2.
Private N_PESOS      As Long
Private CU_COL_SEP   As Long
Private CU_COL_W1    As Long
Private CU_COL_SEP2  As Long
Private CU_COL_MEN   As Long
Private CU_COL_ANN   As Long
Private CU_COL_UMF   As Long
Private CU_COL_UMR   As Long
Private CU_COL_SUB   As Long
Private CU_COL_TIPO  As Long
Private CU_CORTE     As String
...
Public Sub CrearConfig()
    Dim ws As Worksheet, scrPrev As Boolean
    Dim prev As Object, k As Variant
    ...
    For Each k In Array("C2", "C3", "C5", "C6", "C10", "C11", "C12", "C13", "C14", "C15")
    ...
    EscribeCfg ws, 2, "Libro (TRAD / ALT)", Rec(prev, "C2", "TRAD"), _
               "TRAD = solo estrategia F. Trad. ALT = todas las demas."
    EscribeCfg ws, 3, "Ruta del libro de marcas", Rec(prev, "C3", ""), _
               "Vacio = pregunta al ejecutar ActualizarTodo."
    EscribeCfg ws, 4, "Hoja fuente", "3", "Nombre o indice de la hoja."
    EscribeCfg ws, 5, "Inicio del historico", Rec(prev, "C5", DateSerial(2025, 1, 1)), _
               "Se ignoran las marcas anteriores."
    EscribeCfg ws, 6, "Fecha de corte", Rec(prev, "C6", Empty), _
    ...
    EscribeCfg ws, 10, "Base del FY", Rec(prev, "C10", DateSerial(2025, 10, 31)), _
               "FY = 01 nov a 31 oct."
    EscribeCfg ws, 11, "Inicio del periodo libre (MayoTD)", Rec(prev, "C11", DateSerial(2026, 5, 1)), _
               "La base del periodo es esta fecha menos un dia."
    EscribeCfg ws, 12, "Carpeta de archivo", Rec(prev, "C12", ""), _
               "Vacio = subcarpeta Resumenes junto a este libro."
    EscribeCfg ws, 13, "Formato de archivo", Rec(prev, "C13", "XLSX"), "XLSX / PDF / AMBOS / NO"
    EscribeCfg ws, 14, "Filas de grupo", Rec(prev, "C14", "PROMEDIO"), _
               "PROMEDIO = promedio ponderado del bloque.  CONTRIBUCION = solo el suma producto."
    EscribeCfg ws, 15, "Peso del grupo", Rec(prev, "C15", "TODOS"), _
               "TODOS (F1+F2, y F3 si el libro es ALT) / F1 / F2 / F3."

    ws.Range("B17").Value = "MAPA DE COLUMNAS DE LA FUENTE (letras)"
    ws.Range("B17").Font.Bold = True
    ws.Range("B18").Value = "Campo"
    ws.Range("C18").Value = "Letra"
    ws.Range("D18").Value = "Nota"
    ws.Range("B18:D18").Font.Bold = True

    EscribeMap ws, 19, "Nombre", "A", "Etiqueta del fondo"
    EscribeMap ws, 20, "Fecha (Dia de la marca)", "B", "Eje de todo"
    EscribeMap ws, 21, "Fecha EEFF", "C", "Solo trazabilidad"
    EscribeMap ws, 22, "Var SBS", "E", "Se usa solo si Var Adj viene vacio"
    EscribeMap ws, 23, "Var Adj", "F", "Retorno del fondo"
    EscribeMap ws, 24, "Codigo SBS", "G", "Llave, detecta renombres"
    EscribeMap ws, 25, "GAP PRF F1", "I", "Peso propio en Fondo 1"
    EscribeMap ws, 26, "GAP PRF F2", "S", "Peso propio en Fondo 2"
    EscribeMap ws, 27, "GAP PRF F3", "AC", "No se usa en este cuadro"
    EscribeMap ws, 28, "Basico F1", "N", "No se usa en este cuadro"
    EscribeMap ws, 29, "Basico F2", "X", "No se usa en este cuadro"
    EscribeMap ws, 30, "Basico F3", "AH", "No se usa en este cuadro"
    EscribeMap ws, 31, "Estrategia (codigo)", "CF", "Ruteo TRAD / ALT"
    EscribeMap ws, 32, "Exposicion", "CG", "Geografia. No se usa en el cuadro"
    EscribeMap ws, 33, "Vintage", "CH", "Sale en el cuadro, al lado del nombre"
    EscribeMap ws, 34, "Moneda", "CI", "Bloque del libro TRAD"

    ws.Range("B2:B15").HorizontalAlignment = xlLeft
    ...
    ws.Range("C2:C15").Interior.Color = RGB(255, 242, 204)
    ws.Range("C19:C34").Interior.Color = RGB(255, 242, 204)

    CrearClasif

    Application.ScreenUpdating = scrPrev
    MsgBox "Config y Clasif listas." & vbCrLf & vbCrLf & _
           "C2 = " & ws.Range("C2").Value & "   |   C14 = " & ws.Range("C14").Value & _
           "   |   C15 = " & ws.Range("C15").Value & vbCrLf & _
           "Pon la ruta en C3 y el corte en C6, y ejecuta ActualizarTodo.", _
           vbInformation, "CrearConfig"
    Exit Sub
...
Private Function Rec(prev As Object, celda As String, porDefecto As Variant) As Variant
    ' valor previo de Config si existe y no esta vacio, si no el default
    If prev.Exists(celda) Then
        If Not IsEmpty(prev(celda)) Then
            If Len(Trim$(TextoSeguro(prev(celda)))) > 0 Then
                Rec = prev(celda)
                Exit Function
            End If
        End If
...
Public Sub ActualizarTodo()
    ...
    Dim codPorNom As Object, nomPorCod As Object
    ...
    Set codPorNom = CreateObject("Scripting.Dictionary"): codPorNom.CompareMode = 1
    Set nomPorCod = CreateObject("Scripting.Dictionary"): nomPorCod.CompareMode = 1
    ConstruirBD v, colMap, libro, wsCfg, nTrad, nAlt, nSin, nCarg, sinCodigos, avisos, _
                codPorNom, nomPorCod

    Paso "Escribiendo el Mapa"
    EscribirMapa colMap, cabeceras, libro, nTrad, nAlt, nSin, nCarg, sinCodigos, avisos, _
                 codPorNom, nomPorCod
    ...
    MsgBox "v3.2 - ActualizarTodo terminado en " & Format$(Timer - mT0, "0.0") & " s." & vbCrLf & vbCrLf & _
           sNotaFuente & _
           mPasos & vbCrLf & _
...
Falla:
    ...
    texto = "v3.2 - ActualizarTodo se detuvo." & vbCrLf & vbCrLf & msg & vbCrLf & vbCrLf & _
...
Private Function LeerMapaColumnas(wsCfg As Worksheet) As Object
    ...
    For Each k In req
        If Not d.Exists(CStr(k)) Then
            Err.Raise vbObjectError + 10, , "Falta la letra de '" & k & "' en Config (C19:C34)."
        End If
    Next k
...
Private Sub ConstruirBD(v As Variant, colMap As Object, libro As String, _
                        ...
                        sinCodigos As Object, avisos As Object, _
                        codPorNom As Object, nomPorCod As Object)
    ...
    Dim monedas As Object, fuenteMon As Object, conteoMon As Object
    Dim forzados As Object, sMonEf As String

    Set cla = LeerClasif(claOrden)
    Set dupKey = CreateObject("Scripting.Dictionary")
    dupKey.CompareMode = 1

    ' correcciones manuales de moneda (hoja Monedas)
    Set monedas = LeerMonedas()
    Set fuenteMon = CreateObject("Scripting.Dictionary"): fuenteMon.CompareMode = 1
    Set conteoMon = CreateObject("Scripting.Dictionary"): conteoMon.CompareMode = 1
    Set forzados = CreateObject("Scripting.Dictionary"): forzados.CompareMode = 1
    Dim todosFondos As Object
    Set todosFondos = NuevoDic()
    ...
    For i = 1 To n
        ...
        ' --- bloque y subgrupo ---
        If libro = "TRAD" Then
            ' la moneda de la fuente se registra siempre; la hoja Monedas manda si tiene valor
            If Not fuenteMon.Exists(sNom) Then Set fuenteMon(sNom) = NuevoDic()
            fuenteMon(sNom)(UCase$(sMon)) = 1
            If Not conteoMon.Exists(sNom) Then conteoMon(sNom) = 0
            conteoMon(sNom) = conteoMon(sNom) + 1

            sMonEf = sMon
            If monedas.Exists(sNom) Then
                sMonEf = monedas(sNom)
                forzados(sNom) = 1
                flag = AgregaFlag(flag, "Moneda corregida en la hoja Monedas: " & _
                                         UCase$(sMon) & " -> " & UCase$(sMonEf))
            End If

            sBloque = IIf(Len(sMonEf) = 0, "SIN MONEDA", UCase$(sMonEf))
            sSub = ""
            If Len(sMonEf) = 0 Then flag = AgregaFlag(flag, "SIN MONEDA")
        Else
            sBloque = cla(sEstr)(2)
            sSub = cla(sEstr)(1)
        ...
        ' --- nombre <-> codigo SBS, para detectar renombres y clases ---
        If Not codPorNom.Exists(sNom) Then Set codPorNom(sNom) = NuevoDic()
        codPorNom(sNom)(sCod) = 1
        If Len(sCod) > 0 Then
            If Not nomPorCod.Exists(sCod) Then Set nomPorCod(sCod) = NuevoDic()
            nomPorCod(sCod)(sNom) = 1
        End If
        ...
        out(fila, 9) = sExp
        out(fila, 10) = sMon
        If EsNumero(v(i, colMap("Vintage"))) Then
            out(fila, 11) = CDbl(v(i, colMap("Vintage")))
        Else
            out(fila, 11) = sVint
        End If
        out(fila, 12) = dVar
...
Siguiente:
    ...
    DefinirNombre "bdVint", SH_BD, "$K$2:$K$" & ultBD
    ...
    If libro = "TRAD" Then ActualizarHojaMonedas fuenteMon, conteoMon, codPorNom
    ActualizarHojaDatos todosFondos

    ' --- avisos para el Mapa ---
    avisos("Fondos con moneda corregida (hoja Monedas)") = forzados.Count
...
Private Function LeerDatos(ByRef fuera As Object) As Object
    ' Hoja Datos: A fondo, B gross yield, C "SI" para sacarlo del Total,
    ' D vintage manual (opcional, pisa el de la fuente).
    ' Devuelve un diccionario Nombre -> Array(grossYield, vintage) y llena "fuera".
    Dim ws As Worksheet, d As Object, i As Long, ult As Long
    Dim nom As String, gy As Variant, vi As Variant

    Set d = NuevoDic()
    Set fuera = NuevoDic()
    If Not HojaExiste(SH_DAT) Then
        Set LeerDatos = d
        Exit Function
    End If

    Set ws = ThisWorkbook.Worksheets(SH_DAT)
    ult = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    For i = 2 To ult
        nom = Trim$(TextoSeguro(ws.Cells(i, 1).Value))
        If Len(nom) > 0 Then
            gy = NumOVacio(ws.Cells(i, 2).Value)
            vi = NumOVacio(ws.Cells(i, 4).Value)
            d(nom) = Array(gy, vi)
            If UCase$(Left$(Trim$(TextoSeguro(ws.Cells(i, 3).Value)), 2)) = "SI" Then fuera(nom) = 1
        End If
    Next i
...
Private Sub ActualizarHojaDatos(todosFondos As Object)
    ' Append-only: agrega los fondos nuevos al final y no toca nunca B, C ni D.
    Dim ws As Worksheet, existentes As Object
    Dim i As Long, ult As Long, r As Long, k As Variant, nom As String

    Set ws = HojaOCrea(SH_DAT)

    If Len(TextoSeguro(ws.Range("A1").Value)) = 0 Then
        ws.Range("A1").Value = "Fondo"
        ws.Range("B1").Value = "Gross Yield"
        ws.Range("C1").Value = "Fuera del Total"
        ws.Range("D1").Value = "Vintage (manual)"
        ws.Range("A1:D1").Font.Bold = True
        ws.Range("A1:D1").Interior.Color = RGB(217, 217, 217)
        ws.Range("F1").Value = "B: el gross yield del fondo, en %. Lo cargas tu; vacio = celda en blanco en el cuadro."
        ws.Range("F2").Value = "C: escribe SI para que el fondo salga del bloque y aparezca abajo, fuera del Total."
        ws.Range("F3").Value = "D: solo si quieres pisar el vintage de la fuente. Vacio = se usa el de la marca."
        ws.Range("F4").Value = "Esta hoja no se borra: los fondos nuevos se agregan al final."
        ws.Range("F1:F4").Font.Color = RGB(128, 128, 128)
    End If
    ...
    For i = 2 To ult
        nom = Trim$(TextoSeguro(ws.Cells(i, 1).Value))
        If Len(nom) > 0 Then existentes(nom) = i
    Next i

    r = ult
    For Each k In todosFondos.Keys
        If Not existentes.Exists(CStr(k)) Then
            r = r + 1
            ws.Cells(r, 1).Value = CStr(k)
        End If
    Next k

    DefinirNombre "dtNom", SH_DAT, "$A$2:$A$" & Application.WorksheetFunction.Max(2, r)
    DefinirNombre "dtGY", SH_DAT, "$B$2:$B$" & Application.WorksheetFunction.Max(2, r)
    DefinirNombre "dtVint", SH_DAT, "$D$2:$D$" & Application.WorksheetFunction.Max(2, r)

    ws.Columns("A").ColumnWidth = 44
    ws.Columns("B").ColumnWidth = 14
    ws.Columns("C").ColumnWidth = 16
    ws.Columns("D").ColumnWidth = 16
    ws.Columns("F").ColumnWidth = 92
    If r >= 2 Then
        ws.Range("B2:B" & r).NumberFormat = "0.00%"
        ws.Range("C2:D" & r).HorizontalAlignment = xlCenter
        ws.Range("B2:B" & r).Interior.Color = RGB(255, 242, 204)
    End If
End Sub


Private Function LeerMonedas() As Object
    ' Hoja Monedas: columna A el fondo, columna B la moneda correcta.
    ' Solo se leen las filas con algo escrito en B; lo demas usa la fuente.
    Dim ws As Worksheet, d As Object, i As Long, ult As Long
    Dim nom As String, mon As String

    Set d = CreateObject("Scripting.Dictionary"): d.CompareMode = 1
    If Not HojaExiste(SH_MON) Then
        Set LeerMonedas = d
        Exit Function
    End If
    ...
    For i = 2 To ult
        nom = Trim$(TextoSeguro(ws.Cells(i, 1).Value))
        mon = UCase$(Trim$(TextoSeguro(ws.Cells(i, 2).Value)))
        If Len(nom) > 0 And Len(mon) > 0 Then d(nom) = mon
    Next i
...
Private Sub ActualizarHojaMonedas(fuenteMon As Object, conteoMon As Object, _
                                  codPorNom As Object)
    ' Agrega los fondos nuevos y refresca las columnas informativas.
    ' NUNCA toca la columna B: ahi van tus correcciones.
    Dim ws As Worksheet, existentes As Object
    Dim i As Long, ult As Long, r As Long, k As Variant, nom As String

    Set ws = HojaOCrea(SH_MON)

    If Len(TextoSeguro(ws.Range("A1").Value)) = 0 Then
        ws.Range("A1").Value = "Fondo"
        ws.Range("B1").Value = "Moneda correcta"
        ws.Range("C1").Value = "Moneda en la fuente (col CI)"
        ws.Range("D1").Value = "Marcas"
        ws.Range("A1:D1").Font.Bold = True
        ws.Range("A1:D1").Interior.Color = RGB(217, 217, 217)
        ws.Range("F1").Value = "Escribe PEN o USD en la columna B solo para los fondos mal clasificados."
        ws.Range("F2").Value = "Vacio = se usa la moneda de la fuente. Despues ejecuta AplicarMonedas."
        ws.Range("F3").Value = "Esta hoja no se borra: los fondos nuevos se agregan al final."
        ws.Range("F1:F3").Font.Color = RGB(128, 128, 128)
    End If

    Set existentes = CreateObject("Scripting.Dictionary"): existentes.CompareMode = 1
    ult = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    If ult < 1 Then ult = 1
    For i = 2 To ult
        nom = Trim$(TextoSeguro(ws.Cells(i, 1).Value))
    ...
    r = ult
    For Each k In fuenteMon.Keys
        nom = CStr(k)
        If existentes.Exists(nom) Then
            i = CLng(existentes(nom))
        Else
            r = r + 1
            i = r
            ws.Cells(i, 1).Value = nom
        End If
        ws.Cells(i, 3).Value = UnirClaves(fuenteMon(nom))
        ws.Cells(i, 4).Value = conteoMon(nom)
        If fuenteMon(nom).Count > 1 Then
            ' el mismo fondo trae mas de una moneda en la fuente: se parte en dos bloques
            ws.Cells(i, 3).Interior.Color = RGB(255, 199, 206)
            ws.Cells(i, 3).Font.Color = RGB(156, 0, 6)
        Else
            ws.Cells(i, 3).Interior.Pattern = xlNone
            ws.Cells(i, 3).Font.Color = RGB(0, 0, 0)
        End If
        If codPorNom.Exists(nom) Then ws.Cells(i, 5).Value = "'" & UnirClaves(codPorNom(nom))
        If Len(TextoSeguro(ws.Cells(i, 2).Value)) > 0 Then
            ws.Cells(i, 2).Interior.Color = RGB(255, 235, 156)
        Else
            ws.Cells(i, 2).Interior.Pattern = xlNone
        End If
    Next k

    ws.Range("E1").Value = "Codigo(s) SBS"
    ws.Range("E1").Font.Bold = True
    ws.Range("E1").Interior.Color = RGB(217, 217, 217)
    ...
    ws.Range("B2:B" & Application.WorksheetFunction.Max(2, r)).HorizontalAlignment = xlCenter
End Sub


Public Sub AplicarMonedas()
    ' Reescribe el Bloque de la BD segun la hoja Monedas y rearma el cuadro.
    ' No va a la red: son segundos.
    Dim wsBD As Worksheet, monedas As Object
    Dim ult As Long, i As Long, n As Long
    Dim nom As String, libro As String

    On Error GoTo Falla

    libro = UCase$(Trim$(TextoSeguro(ThisWorkbook.Worksheets(SH_CFG).Range("C2").Value)))
    If libro <> "TRAD" Then
        MsgBox "La hoja Monedas solo aplica al libro TRAD (el bloque es la moneda)." & vbCrLf & _
               "Este libro esta en " & libro & ".", vbInformation, "AplicarMonedas"
        Exit Sub
    End If

    If Not HojaExiste(SH_BD) Then
        MsgBox "No hay BD cargada. Ejecuta ActualizarTodo.", vbExclamation, "AplicarMonedas"
        Exit Sub
    End If
    ...
    If monedas.Count = 0 Then
        MsgBox "La columna B de la hoja Monedas esta vacia, asi que no hay nada que aplicar." & vbCrLf & vbCrLf & _
               "Escribe PEN o USD en la columna B (al lado del nombre del fondo)," & vbCrLf & _
               "guarda, y vuelve a ejecutar AplicarMonedas.", vbExclamation, "AplicarMonedas"
        Exit Sub
    End If

    Set wsBD = ThisWorkbook.Worksheets(SH_BD)
    ult = wsBD.Cells(wsBD.Rows.Count, 1).End(xlUp).Row

    Dim vistos As Object, k As Variant, noHallados As String
    Set vistos = NuevoDic()

    Application.ScreenUpdating = False
    For i = 2 To ult
        nom = Trim$(TextoSeguro(wsBD.Cells(i, 4).Value))
        If monedas.Exists(nom) Then
            vistos(nom) = 1
            If TextoSeguro(wsBD.Cells(i, 6).Value) <> UCase$(CStr(monedas(nom))) Then
                wsBD.Cells(i, 6).Value = UCase$(CStr(monedas(nom)))
                n = n + 1
            End If
        End If
    Next i
    ...
    For Each k In monedas.Keys
        If Not vistos.Exists(CStr(k)) Then
            noHallados = noHallados & vbCrLf & "   " & CStr(k)
        End If
    Next k

    Recalcular

    MsgBox "Correcciones leidas de la columna B: " & monedas.Count & vbCrLf & _
           "Fondos encontrados en la BD: " & vistos.Count & vbCrLf & _
           "Marcas reclasificadas: " & n & vbCrLf & _
           IIf(n = 0 And vistos.Count > 0, _
               vbCrLf & "Ya estaban en esa moneda: no habia nada que cambiar." & vbCrLf, "") & _
           IIf(Len(noHallados) > 0, _
               vbCrLf & "Estos nombres de la hoja Monedas NO estan en la BD" & vbCrLf & _
               "(revisa que esten escritos igual):" & noHallados, ""), _
           vbInformation, "AplicarMonedas"
    Exit Sub

Falla:
    Application.ScreenUpdating = True
    MsgBox "AplicarMonedas se detuvo." & vbCrLf & vbCrLf & Err.Description, vbCritical, "AplicarMonedas"
End Sub
...
Private Sub EscribirMapa(colMap As Object, cabeceras As Object, libro As String, _
    ...
    For Each k In avisos.Keys
        ...
        Else
            ...
            If avisos(k) > 0 _
               ...
               And CStr(k) <> "Filas saltadas: anteriores al inicio del historico" _
               And CStr(k) <> "Fondos con moneda corregida (hoja Monedas)" Then
    ...
    ' --- mismo codigo con varios nombres / mismo nombre con varios codigos ---
    Dim nMulti As Long
    For Each k In nomPorCod.Keys
        If nomPorCod(k).Count > 1 Then nMulti = nMulti + 1
    Next k

    r = r + 2
    ws.Cells(r, 1).Value = "UN MISMO CODIGO SBS CON VARIOS NOMBRES"
    ws.Cells(r, 1).Font.Bold = True
    r = r + 1
    ws.Cells(r, 1).Value = "Si aparece algo aqui, la fuente renombro el fondo y su historia " & _
                           "se esta partiendo en dos filas del cuadro."
    ws.Cells(r, 1).Font.Color = RGB(128, 128, 128)
    If nMulti = 0 Then
        r = r + 1
        ws.Cells(r, 1).Value = "Ninguno. Cada codigo SBS tiene un solo nombre."
    Else
        For Each k In nomPorCod.Keys
            If nomPorCod(k).Count > 1 Then
                r = r + 1
                ws.Cells(r, 1).Value = "Codigo " & CStr(k)
                ws.Cells(r, 2).Value = nomPorCod(k).Count & " nombres"
                ws.Cells(r, 3).Value = UnirClaves(nomPorCod(k))
    ...
    r = r + 2
    ws.Cells(r, 1).Value = "UN MISMO NOMBRE CON VARIOS CODIGOS SBS"
    ws.Cells(r, 1).Font.Bold = True
    nMulti = 0
    For Each k In codPorNom.Keys
        If codPorNom(k).Count > 1 Then
            nMulti = nMulti + 1
            r = r + 1
            ws.Cells(r, 1).Value = CStr(k)
            ws.Cells(r, 2).Value = codPorNom(k).Count & " codigos"
            ws.Cells(r, 3).Value = UnirClaves(codPorNom(k))
            ws.Range(ws.Cells(r, 1), ws.Cells(r, 3)).Interior.Color = RGB(255, 235, 156)
        End If
    Next k
    If nMulti = 0 Then
        r = r + 1
        ws.Cells(r, 1).Value = "Ninguno. Cada nombre tiene un solo codigo SBS."
    End If
...
Private Sub ArmarCuadro(libro As String)
    ...
    Marca "ArmarCuadro: leyendo Clasif y Datos"
    Set cla = LeerClasif(claOrden)
    Set datos = LeerDatos(fuera)

    Marca "ArmarCuadro: armando la estructura de filas"
    Set fondosFuera = NuevoDic()
    Set bloques = ArmarEstructura(wsBD, ultBD, libro, dCorte, dBaseMin, claOrden, nExcl, _
                                  fuera, fondosFuera)
    ...
    ws.Range(CU_CORTE).Value = dCorte
    ws.Range(CU_CORTE).NumberFormat = "dd/mm/yyyy"
    ws.Range(CU_CORTE).Font.Bold = True
    ws.Cells(2, CU_COL_W1 + 1).Value = "<- fecha de corte (Config!C6, vacio = ultima marca)"
    ws.Cells(2, CU_COL_W1 + 1).Font.Color = RGB(128, 128, 128)

    ws.Range("B3").Value = "Fondos sin posicion excluidos: " & nExcl & _
        "   |   Retorno compuesto sobre Var Adj   |   Filas de grupo: " & _
        ModoGrupo() & " ponderado por " & PesoGrupo()
    ...
    EscribirFilas ws, bloques, fondosFuera, ultFila
...
Private Sub FijarLayout(libro As String)
    ' TRAD: F1 F2   |   ALT: F1 F2 F3
    If UCase$(Trim$(libro)) = "ALT" Then
        N_PESOS = 3
    Else
        N_PESOS = 2
    End If
    CU_COL_SEP = CU_COL_P1 + N_PESOS
    CU_COL_W1 = CU_COL_SEP + 1
    CU_COL_SEP2 = CU_COL_W1 + N_VENT
    CU_COL_MEN = CU_COL_SEP2 + 1
    CU_COL_ANN = CU_COL_MEN + 1
    CU_COL_UMF = CU_COL_ANN + 1
    CU_COL_UMR = CU_COL_UMF + 1
    CU_COL_SUB = CU_COL_UMR + 1
    CU_COL_TIPO = CU_COL_SUB + 1
    CU_CORTE = "$" & NumALetra(CU_COL_W1) & "$2"
End Sub


Private Function ModoGrupo() As String
    Dim m As String
    m = UCase$(Trim$(TextoSeguro(ThisWorkbook.Worksheets(SH_CFG).Range("C14").Value)))
    If m <> "CONTRIBUCION" Then m = "PROMEDIO"
    ModoGrupo = m
End Function


Private Function PesoGrupo() As String
    Dim m As String
    m = UCase$(Replace(Trim$(TextoSeguro(ThisWorkbook.Worksheets(SH_CFG).Range("C15").Value)), " ", ""))
    If m <> "F1" And m <> "F2" And m <> "F3" Then m = "TODOS"
    PesoGrupo = m
End Function
Private Sub EscribirEncabezadoVentanas(ws As Worksheet)
    Dim nom As Variant, j As Long, c As String

    nom = Array("WTD", "5D", "MTD", "MayoTD", "YTD", "FY")

    ws.Cells(CU_FILA_FIN, CU_COL_NOM).Value = "fin"
    ws.Cells(CU_FILA_BASE, CU_COL_NOM).Value = "base"
    ws.Range(ws.Cells(CU_FILA_FIN, CU_COL_NOM), ws.Cells(CU_FILA_BASE, CU_COL_NOM)).HorizontalAlignment = xlRight
    ws.Range(ws.Cells(CU_FILA_FIN, CU_COL_NOM), ws.Cells(CU_FILA_BASE, CU_COL_NOM)).Font.Color = RGB(128, 128, 128)

    For j = 0 To N_VENT - 1
        ws.Cells(CU_FILA_FIN, CU_COL_W1 + j).Formula = "=" & CU_CORTE
        ws.Cells(CU_FILA_CAB, CU_COL_W1 + j).Value = nom(j)
    Next j

    c = CU_CORTE
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 0).Formula = "=" & c & "-WEEKDAY(" & c & ",3)-1"
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 1).Formula = "=" & c & "-" & SH_CFG & "!$C$9"
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 2).Formula = "=EOMONTH(" & c & ",-1)"
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 3).Formula = "=" & SH_CFG & "!$C$11-1"
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 4).Formula = "=DATE(YEAR(" & c & "),1,1)-1"
    ws.Cells(CU_FILA_BASE, CU_COL_W1 + 5).Formula = "=" & SH_CFG & "!$C$10"

    ws.Cells(CU_FILA_CAB, CU_COL_VIN).Value = "Vintage"
    ws.Cells(CU_FILA_CAB, CU_COL_GY).Value = "Gross Yield"
    For j = 0 To N_PESOS - 1
        ws.Cells(CU_FILA_CAB, CU_COL_P1 + j).Value = "F" & (j + 1)
    Next j
    ws.Cells(CU_FILA_CAB, CU_COL_MEN).Value = "Mensual YTD"
    ws.Cells(CU_FILA_CAB, CU_COL_ANN).Value = "Ann YTD"
    ws.Cells(CU_FILA_CAB, CU_COL_UMF).Value = "Ult. marca"
    ws.Cells(CU_FILA_CAB, CU_COL_UMR).Value = "Ret. ult. marca"
End Sub
Private Sub EscribirFilas(ws As Worksheet, bloques As Object, fondosFuera As Object, _
                          ByRef ultFila As Long)
    Dim r As Long, j As Long
    Dim kB As Variant, kS As Variant, kF As Variant
    Dim subs As Object, fondos As Object
    Dim grupos As Object          ' fila -> tipo (BLQ / SUB)
    Dim filaTot As Long, r1 As Long

    Set grupos = NuevoDic()
    r1 = CU_FILA_DAT
    r = CU_FILA_DAT - 1

    ' ---------- pasada 1: etiquetas y filas de fondo ----------
    For Each kB In bloques.Keys
        ...
        ws.Cells(r, CU_COL_TIPO).Value = "BLQ"
        ...
        For Each kS In subs.Keys
            If TextoSeguro(kS) <> "" Then
                ...
                ws.Cells(r, CU_COL_TIPO).Value = "SUB"
            End If

            Set fondos = subs(kS)
            For Each kF In fondos.Keys
                r = r + 1
                EscribirFilaFondo ws, r, TextoSeguro(kB), TextoSeguro(kS), TextoSeguro(kF), "FND"
            Next kF
        Next kS
    Next kB
    ...
    ws.Cells(r, CU_COL_TIPO).Value = "TOT"

    ' ---------- fondos fuera del Total ----------
    If fondosFuera.Count > 0 Then
        r = r + 1                                  ' fila en blanco de separacion
        r = r + 1
        ws.Cells(r, CU_COL_NOM).Value = "Fuera del Total"
        ws.Cells(r, CU_COL_NOM).Font.Italic = True
        ws.Cells(r, CU_COL_NOM).Font.Color = RGB(128, 128, 128)
        ws.Cells(r, CU_COL_NOM).Font.Size = 8
        ws.Cells(r, CU_COL_TIPO).Value = "SEP"
        For Each kF In fondosFuera.Keys
            r = r + 1
            EscribirFilaFondo ws, r, "", "", CStr(kF), "OUT"
        Next kF
    End If

    ultFila = r

    ' ---------- pasada 2: filas de grupo y Total, ya con el rango completo ----------
    For Each kF In grupos.Keys
        Marca "Filas: formulas del grupo en la fila " & kF
        EscribirFilaGrupo ws, CLng(kF), CStr(grupos(kF)), r1, ultFila
    Next kF
    Marca "Filas: formulas de la fila Total"
    EscribirFilaGrupo ws, filaTot, "TOT", r1, ultFila
End Sub


Private Sub EscribirFilaFondo(ws As Worksheet, r As Long, bloque As String, subg As String, _
                              nombre As String, tipo As String)
    Dim j As Long, colYTD As String

    ws.Cells(r, CU_COL_BLQ).Value = bloque
    ws.Cells(r, CU_COL_SUB).Value = subg
    ws.Cells(r, CU_COL_NOM).Value = nombre
    ws.Cells(r, CU_COL_TIPO).Value = tipo
    If tipo = "FND" Then ws.Cells(r, CU_COL_NOM).IndentLevel = 2

    ' vintage: el de la ultima marca <= corte, salvo que la hoja Datos lo pise
    ws.Cells(r, CU_COL_VIN).Formula = _
        "=IFERROR(IF(N(INDEX(dtVint,MATCH($B" & r & ",dtNom,0)))<>0," & _
        "INDEX(dtVint,MATCH($B" & r & ",dtNom,0))," & _
        "AVERAGEIFS(bdVint,bdNom,$B" & r & ",bdDia," & MaxDia(r) & ")),"""")"

    ' gross yield: solo de la hoja Datos
    ws.Cells(r, CU_COL_GY).Formula = _
        "=IFERROR(INDEX(dtGY,MATCH($B" & r & ",dtNom,0)),"""")"

    ' pesos: 0 en vez de vacio para que el suma producto de las filas grises funcione
    For j = 0 To N_PESOS - 1
        ws.Cells(r, CU_COL_P1 + j).Formula = _
            "=IFERROR(AVERAGEIFS(bdPeso" & (j + 1) & ",bdNom,$B" & r & ",bdDia," & MaxDia(r) & "),0)"
    Next j

    For j = 0 To N_VENT - 1
        ws.Cells(r, CU_COL_W1 + j).Formula = FormulaRetorno(r, CU_COL_W1 + j)
    Next j

    colYTD = NumALetra(CU_COL_W1 + 4)
    ws.Cells(r, CU_COL_MEN).Formula = FormulaPeriodo(r, colYTD, "30.4375")
    ws.Cells(r, CU_COL_ANN).Formula = FormulaPeriodo(r, colYTD, "365")

    ' cuando fue la ultima marca y cuanto rindio ese periodo.
    ' El Var Adj de una marca ES el retorno desde la marca anterior, asi que no
    ' hay que componer nada: es el retorno del periodo que esa marca cierra.
    ws.Cells(r, CU_COL_UMF).Formula = _
        "=IFERROR(IF(" & MaxDia(r) & "=0,""""," & MaxDia(r) & "),"""")"
    ws.Cells(r, CU_COL_UMR).Formula = _
        "=IFERROR(AVERAGEIFS(bdVar,bdNom,$B" & r & ",bdDia," & MaxDia(r) & "),"""")"
End Sub


Private Function MaxDia(r As Long) As String
    MaxDia = "MAXIFS(bdDia,bdNom,$B" & r & ",bdDia,""<=""&" & CU_CORTE & ")"
End Function


Private Function FormulaPeriodo(r As Long, colYTD As String, dias As String) As String
    ' (1+YTD)^(dias/d)-1  con d = dias entre la base y el fin de la ventana YTD
    FormulaPeriodo = "=IFERROR(IF(" & colYTD & "$" & CU_FILA_BASE & ">=" & colYTD & "$" & CU_FILA_FIN & _
                     ",NA(),(1+" & colYTD & r & ")^(" & dias & "/(" & colYTD & "$" & CU_FILA_FIN & _
                     "-" & colYTD & "$" & CU_FILA_BASE & "))-1),NA())"
End Function


Private Sub EscribirFilaGrupo(ws As Worksheet, r As Long, tipo As String, r1 As Long, r2 As Long)
    Dim j As Long, w As String, cond As String, colL As String

    w = ExprPeso(r1, r2)
    cond = ExprCond(tipo, r, r1, r2)

    ' pesos: suma simple de los fondos del grupo
    For j = 0 To N_PESOS - 1
        ws.Cells(r, CU_COL_P1 + j).Formula = SumaPesoGrupo(tipo, r, r1, r2, CU_COL_P1 + j)
    Next j

    ' gross yield: el denominador cuenta solo los fondos que tienen yield cargado
    ws.Cells(r, CU_COL_GY).Formula = FormulaGrupoGY(w, cond, r1, r2)

    For j = 0 To N_VENT - 1
        colL = NumALetra(CU_COL_W1 + j)
        ws.Cells(r, CU_COL_W1 + j).Formula = _
            "=IF(" & colL & "$" & CU_FILA_BASE & ">=" & colL & "$" & CU_FILA_FIN & ",NA()," & _
            CuerpoGrupo(w, cond, CU_COL_W1 + j, r1, r2) & ")"
    Next j

    ws.Cells(r, CU_COL_MEN).Formula = "=" & CuerpoGrupo(w, cond, CU_COL_MEN, r1, r2)
    ws.Cells(r, CU_COL_ANN).Formula = "=" & CuerpoGrupo(w, cond, CU_COL_ANN, r1, r2)

    ' Ult. marca y su retorno quedan VACIAS en las filas de grupo: cada fondo
    ' marca en fecha distinta, promediar retornos de periodos distintos no dice nada.
    ws.Cells(r, CU_COL_UMF).ClearContents
    ws.Cells(r, CU_COL_UMR).ClearContents
End Sub


Private Function CuerpoGrupo(w As String, cond As String, col As Long, r1 As Long, r2 As Long) As String
    Dim num As String
    num = "SUMPRODUCT(" & w & "*" & cond & "," & RngFilasAbs(col, r1, r2) & ")"
    If ModoGrupo() = "CONTRIBUCION" Then
        CuerpoGrupo = "IFERROR(" & num & ","""")"
    Else
        CuerpoGrupo = "IFERROR(" & num & "/SUMPRODUCT(" & w & "*" & cond & "),"""")"
    End If
End Function


Private Function FormulaGrupoGY(w As String, cond As String, r1 As Long, r2 As Long) As String
    Dim rg As String, num As String
    rg = RngFilasAbs(CU_COL_GY, r1, r2)
    num = "SUMPRODUCT(" & w & "*" & cond & "," & rg & ")"
    If ModoGrupo() = "CONTRIBUCION" Then
        FormulaGrupoGY = "=IFERROR(" & num & ","""")"
    Else
        FormulaGrupoGY = "=IFERROR(" & num & "/SUMPRODUCT(" & w & "*" & cond & _
                         "*(" & rg & "<>"""")),"""")"
    End If
End Function


Private Function SumaPesoGrupo(tipo As String, r As Long, r1 As Long, r2 As Long, col As Long) As String
    Dim rgP As String, rgT As String
    rgP = RngFilasAbs(col, r1, r2)
    rgT = RngAbs(CU_COL_TIPO, r1, r2)
    Select Case tipo
        Case "BLQ"
            SumaPesoGrupo = "=SUMIFS(" & rgP & "," & RngAbs(CU_COL_BLQ, r1, r2) & ",$" & _
                            NumALetra(CU_COL_BLQ) & r & "," & rgT & ",""FND"")"
        Case "SUB"
            SumaPesoGrupo = "=SUMIFS(" & rgP & "," & RngAbs(CU_COL_SUB, r1, r2) & ",$" & _
                            NumALetra(CU_COL_SUB) & r & "," & rgT & ",""FND"")"
        Case Else
            SumaPesoGrupo = "=SUMIFS(" & rgP & "," & rgT & ",""FND"")"
    End Select
End Function


Private Function ExprPeso(r1 As Long, r2 As Long) As String
    Dim j As Long, s As String
    Select Case PesoGrupo()
        Case "F1": ExprPeso = RngAbs(CU_COL_P1, r1, r2)
        Case "F2": ExprPeso = RngAbs(CU_COL_P1 + 1, r1, r2)
        Case "F3": ExprPeso = RngAbs(CU_COL_P1 + 2, r1, r2)
        Case Else
            ' TODOS: la suma de los pesos que tenga este libro (F1+F2, o F1+F2+F3)
            For j = 0 To N_PESOS - 1
                If Len(s) > 0 Then s = s & "+"
                s = s & RngAbs(CU_COL_P1 + j, r1, r2)
            Next j
            ExprPeso = "(" & s & ")"
    End Select
End Function
...
Private Function ExprCond(tipo As String, r As Long, r1 As Long, r2 As Long) As String
    Dim soloFnd As String
    soloFnd = "(" & RngAbs(CU_COL_TIPO, r1, r2) & "=""FND"")"
    Select Case tipo
        Case "BLQ"
            ExprCond = "(" & RngAbs(CU_COL_BLQ, r1, r2) & "=$" & NumALetra(CU_COL_BLQ) & r & ")*" & soloFnd
        Case "SUB"
            ExprCond = "(" & RngAbs(CU_COL_SUB, r1, r2) & "=$" & NumALetra(CU_COL_SUB) & r & ")*" & soloFnd
        Case Else
            ExprCond = soloFnd
    End Select
End Function
...
Private Function RngAbs(col As Long, r1 As Long, r2 As Long) As String
    RngAbs = "$" & NumALetra(col) & "$" & r1 & ":$" & NumALetra(col) & "$" & r2
End Function


Private Function RngFilasAbs(col As Long, r1 As Long, r2 As Long) As String
    ' columna relativa, filas fijas: la formula se puede arrastrar de columna
    RngFilasAbs = NumALetra(col) & "$" & r1 & ":" & NumALetra(col) & "$" & r2
End Function
...
Private Function ArmarEstructura(wsBD As Worksheet, ultBD As Long, libro As String, _
                                 ...
                                 claOrden As Object, ByRef nExcl As Long, _
                                 fuera As Object, fondosFuera As Object) As Object
        ...
        For isb = LBound(arrSubs) To UBound(arrSubs)
            ...
            If IsArray(nombres) Then
                For inm = LBound(nombres) To UBound(nombres)
                    ...
                    If Not conPos.Exists(sN) Then
                        nExcl = nExcl + 1
                    ElseIf fuera.Exists(sN) Then
                        fondosFuera(sN) = 1          ' va abajo, fuera del Total
                    Else
                        fondos(sN) = 1
                    End If
                Next inm
    ...
    If bloques.Count = 0 And fondosFuera.Count = 0 Then
        Err.Raise vbObjectError + 22, , "Ningun fondo paso el filtro de posicion. " & _
                  "Revisa la fecha de corte (Config!C6) y los pesos de las cols I y S."
    End If
...
Private Sub FormatearCuadro(ws As Worksheet, ultFila As Long)
    ...
    Dim tipo As String
    Dim uni As Range, rFila As Range

    If ultFila < CU_FILA_DAT Then Exit Sub

    Marca "Formato: anchos de columna"
    ws.Columns(CU_COL_BLQ).ColumnWidth = 18
    ws.Columns(CU_COL_NOM).ColumnWidth = 42
    ws.Columns(CU_COL_VIN).ColumnWidth = 9
    ws.Columns(CU_COL_GY).ColumnWidth = 12
    ws.Range(NumALetra(CU_COL_P1) & ":" & NumALetra(CU_COL_P1 + N_PESOS - 1)).ColumnWidth = 8
    ws.Columns(CU_COL_SEP).ColumnWidth = 1.6
    ws.Range(NumALetra(CU_COL_W1) & ":" & NumALetra(CU_COL_W1 + N_VENT - 1)).ColumnWidth = 10
    ws.Columns(CU_COL_SEP2).ColumnWidth = 1.6
    ws.Columns(CU_COL_MEN).ColumnWidth = 12
    ws.Columns(CU_COL_ANN).ColumnWidth = 12
    ws.Columns(CU_COL_UMF).ColumnWidth = 11
    ws.Columns(CU_COL_UMR).ColumnWidth = 14
    ...
    With ws.Range(ws.Cells(CU_FILA_FIN, CU_COL_W1), ws.Cells(CU_FILA_BASE, CU_COL_W1 + N_VENT - 1))
        .Font.Size = 8
    ...
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_VIN), ws.Cells(ultFila, CU_COL_VIN)).NumberFormat = "0"
    ' el cero se esconde: en peso y yield significa "no hay", no "es cero"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_GY), ws.Cells(ultFila, CU_COL_P1 + N_PESOS - 1)).NumberFormat = "0.00%;-0.00%;"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_W1), ws.Cells(ultFila, CU_COL_W1 + N_VENT - 1)).NumberFormat = "0.00%"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_MEN), ws.Cells(ultFila, CU_COL_ANN)).NumberFormat = "0.00%"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_UMF), ws.Cells(ultFila, CU_COL_UMF)).NumberFormat = "dd/mm/yy"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_UMR), ws.Cells(ultFila, CU_COL_UMR)).NumberFormat = "0.00%"
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_VIN), ws.Cells(ultFila, CU_COL_UMR)).HorizontalAlignment = xlCenter
    ...
    With ws.Range(ws.Cells(CU_FILA_CAB, CU_COL_W1), ws.Cells(CU_FILA_CAB, CU_COL_W1 + N_VENT - 1))
        ...
        .HorizontalAlignment = xlCenter
    End With
    With ws.Range(ws.Cells(CU_FILA_CAB, CU_COL_MEN), ws.Cells(CU_FILA_CAB, CU_COL_UMR))
        .Interior.Color = RGB(127, 127, 127)
        .Font.Color = RGB(255, 255, 255)
        .Font.Bold = True
        .HorizontalAlignment = xlCenter
    End With
    With ws.Range(ws.Cells(CU_FILA_CAB, CU_COL_VIN), ws.Cells(CU_FILA_CAB, CU_COL_P1 + N_PESOS - 1))
        .Font.Bold = True
        .HorizontalAlignment = xlCenter
    ...
    With ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_NOM), ws.Cells(ultFila, CU_COL_UMR)).Borders
        .LineStyle = xlContinuous
        .Color = RGB(217, 217, 217)
        .Weight = xlThin
    End With
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_SEP), ws.Cells(ultFila, CU_COL_SEP)).Borders.LineStyle = xlNone
    ws.Range(ws.Cells(CU_FILA_DAT, CU_COL_SEP2), ws.Cells(ultFila, CU_COL_SEP2)).Borders.LineStyle = xlNone

    For r = CU_FILA_DAT To ultFila
        Marca "Formato: pintando la fila " & r
        tipo = TextoSeguro(ws.Cells(r, CU_COL_TIPO).Value)
        Set rFila = ws.Range(ws.Cells(r, CU_COL_NOM), ws.Cells(r, CU_COL_UMR))
        Select Case tipo
            Case "BLQ"
                rFila.Interior.Color = RGB(128, 128, 128)
                rFila.Font.Color = RGB(255, 255, 255)
                rFila.Font.Bold = True
                ws.Range(ws.Cells(r, CU_COL_SEP), ws.Cells(r, CU_COL_SEP)).Interior.Pattern = xlNone
                ws.Range(ws.Cells(r, CU_COL_SEP2), ws.Cells(r, CU_COL_SEP2)).Interior.Pattern = xlNone
            Case "SUB"
            ...
            Case "TOT"
                rFila.Font.Bold = True
                With rFila.Borders(xlEdgeTop)
                    .LineStyle = xlContinuous
                    .Color = RGB(64, 64, 64)
                    .Weight = xlMedium
                End With
                With rFila.Borders(xlEdgeBottom)
                    .LineStyle = xlContinuous
                    .Color = RGB(64, 64, 64)
                    .Weight = xlMedium
                End With
            ...
            Case "FND", "OUT"
                If uni Is Nothing Then
                    Set uni = ws.Range(ws.Cells(r, CU_COL_W1), ws.Cells(r, CU_COL_W1))
                End If
        End Select
        If tipo = "" Then rFila.Borders.LineStyle = xlNone
    Next r

    Marca "Formato: escala de color"
    For j = 0 To N_VENT - 1
        AplicarEscala RangoDeTipo(ws, CU_COL_W1 + j, CU_FILA_DAT, ultFila)
    Next j
    AplicarEscala RangoDeTipo(ws, CU_COL_MEN, CU_FILA_DAT, ultFila)
    AplicarEscala RangoDeTipo(ws, CU_COL_ANN, CU_FILA_DAT, ultFila)
    AplicarEscala RangoDeTipo(ws, CU_COL_UMR, CU_FILA_DAT, ultFila)

    Marca "Formato: ocultando columnas auxiliares"
    ws.Columns(CU_COL_BLQ).Hidden = True
    ws.Columns(CU_COL_SUB).Hidden = True
    ws.Columns(CU_COL_TIPO).Hidden = True
...
Private Function RangoDeTipo(ws As Worksheet, col As Long, r1 As Long, r2 As Long) As Range
    ' solo las filas de fondo: la escala de color no debe pintar las filas grises
    Dim r As Long, u As Range, t As String
    For r = r1 To r2
        t = TextoSeguro(ws.Cells(r, CU_COL_TIPO).Value)
        If t = "FND" Or t = "OUT" Then
...
Private Sub AplicarEscala(rng As Range)
    ...
    ' Escala divergente SIMETRICA alrededor de cero: -M rojo, 0 blanco, +M verde.
    ' No se usa "el valor mas bajo de la columna" como extremo rojo, porque cuando
    ' ningun fondo cae, el mas bajo es 0.0% y Excel lo pinta rojo del todo: un fondo
    ' que no marco se veria como el peor de la tabla.
    ' M = percentil 90 de los valores absolutos, para que un dato extremo (un +94%)
    ' no deje al resto de la columna en blanco.
    Dim fc As ColorScale
    Dim c As Range
    Dim abs_() As Double, n As Long, i As Long, j As Long, t As Double
    Dim M As Double, idx As Long

    On Error Resume Next
    rng.FormatConditions.Delete
    On Error GoTo 0

    ' --- recolectar los valores absolutos ---
    ReDim abs_(1 To rng.Cells.Count)
    n = 0
    For Each c In rng.Cells
        If Not IsError(c.Value) Then
            If IsNumeric(c.Value) And Len(CStr(c.Text)) > 0 Then
                n = n + 1
                abs_(n) = Abs(CDbl(c.Value))
            End If
        End If
    Next c
    If n = 0 Then Exit Sub

    ' --- orden ascendente (pocos elementos, insercion simple) ---
    For i = 1 To n - 1
        For j = i + 1 To n
            If abs_(j) < abs_(i) Then
                t = abs_(i): abs_(i) = abs_(j): abs_(j) = t
            End If
        Next j
    Next i
    ...
    If idx > n Then idx = n
    M = abs_(idx)
    If M <= 0 Then M = abs_(n)      ' si el p90 es cero, usar el maximo
    If M <= 0 Then Exit Sub         ' columna entera en cero: sin escala
...
Public Sub ArchivarAhora()
    ...
    With wbNew.Worksheets(1)
        ...
        .Columns(CU_COL_TIPO).Hidden = True
    End With
...
Private Function UnirClaves(d As Object) As String
