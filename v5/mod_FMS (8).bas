...
Private Function ListarCierres(ByRef meses() As String, _
    ...
    EscanearCarpeta fso.GetFolder(carpeta), dArch, dDia
    ...
    Next i
    For i = 1 To n
        archivos(i) = dArch(meses(i))             ' ruta completa
    Next i
    ListarCierres = n
End Function

'--- Recorre una carpeta y sus subcarpetas juntando los FMS ----------
Private Sub EscanearCarpeta(fld As Object, dArch As Object, dDia As Object)
    Dim f As Object, sf As Object
    Dim nom As String, f8 As String, ym As String, mm As Long, dd As Long
    For Each f In fld.Files
        nom = f.Name
        If UCase$(nom) Like "FMS_########.XLS*" Then
            f8 = Mid$(nom, 5, 8)                  ' AAAAMMDD
            ym = Left$(f8, 6)
            mm = CLng(Mid$(f8, 5, 2))
            dd = CLng(Right$(f8, 2))
