...
Private Function ListarCierres(ByRef meses() As String, _
    ...
    carpeta = CarpetaFMS()
    If Right$(carpeta, 1) = "\" Then carpeta = Left$(carpeta, Len(carpeta) - 1)
    If Not fso.FolderExists(carpeta) Then Exit Function

    EscanearCarpeta fso.GetFolder(carpeta), dArch, dDia, dArch2, dDia2
...
Public Sub DiagnosticoCarpeta()
    ...
    If Not fso.FolderExists(carpeta) Then
        MsgBox "No existe la carpeta:" & vbCrLf & carpeta, vbExclamation
        Exit Sub
    End If
    On Error Resume Next
    ...
    fila = 2
    DiagCarpeta fso.GetFolder(carpeta), ws, fila
...
FalloArchivo:
    ...
    informe = informe & nombre & " -> ERROR " & eNum & ": " & eDesc & _
