...
Private Sub Importar(soloUltimo As Boolean)
    ...
    Dim meses() As String, archivos() As String, archivos2() As String
    Dim n As Long, i As Long

    On Error GoTo Manejo
    Set wsD = ObtenerHojaDestino()

    n = ListarCierres(meses, archivos, archivos2)
    If n = 0 Then
    ...
    For i = iIni To n
        If Not CargarArchivo(wsD, meses(i), archivos(i), informe) Then
            If archivos2(i) <> vbNullString Then    ' cierre fallido:
                CargarArchivo wsD, meses(i), archivos2(i), informe
            End If                                  ' reintenta dia anterior
        End If
    Next i
...
Limpieza:
    ...
    Debug.Print informe                           ' informe completo (Ctrl+G)
    Dim lineas() As String, resumen As String, t As Long
    lineas = Split(informe, vbCrLf)
    If UBound(lineas) > 16 Then
        For t = 0 To 15
            resumen = resumen & lineas(t) & vbCrLf
        Next t
        resumen = resumen & "..." & vbCrLf & "(" & UBound(lineas) & _
                  " meses en total; informe completo en la ventana " & _
                  "Inmediato del editor VBA: Alt+F11 y Ctrl+G)"
    Else
        resumen = informe
    End If
    MsgBox resumen, vbInformation, "Importar FMS (Fondo 1 y 2)"
    Exit Sub
...
'--- Divide C2 en varias rutas separadas por ";" ----------------------
Private Function RutasFMS() As Variant
    Dim partes() As String, i As Long
    partes = Split(CarpetaFMS(), ";")
    For i = LBound(partes) To UBound(partes)
        partes(i) = Trim$(partes(i))
        If Len(partes(i)) > 0 Then
            If Right$(partes(i), 1) = "\" Then
                partes(i) = Left$(partes(i), Len(partes(i)) - 1)
            End If
        End If
    Next i
    RutasFMS = partes
...
'--- Escanea la carpeta Y SUS SUBCARPETAS (una por anio) y devuelve,
'    por mes, el archivo de cierre (la fecha AAAAMMDD mas alta de cada
'    AAAAMM), en orden ascendente. Guarda la ruta completa.
Private Function ListarCierres(ByRef meses() As String, _
                               ByRef archivos() As String, _
                               ByRef archivos2() As String) As Long
    Dim fso As Object, dArch As Object, dDia As Object
    Dim dArch2 As Object, dDia2 As Object
    Set fso = CreateObject("Scripting.FileSystemObject")
    Set dArch = CreateObject("Scripting.Dictionary")
    Set dDia = CreateObject("Scripting.Dictionary")
    Set dArch2 = CreateObject("Scripting.Dictionary")
    Set dDia2 = CreateObject("Scripting.Dictionary")

    Dim rutas As Variant, r As Long
    rutas = RutasFMS()
    For r = LBound(rutas) To UBound(rutas)
        If CStr(rutas(r)) <> vbNullString Then
            If fso.FolderExists(rutas(r)) Then
                EscanearCarpeta fso.GetFolder(rutas(r)), dArch, dDia, dArch2, dDia2
            End If
        End If
    Next r
    ...
    ReDim meses(1 To n): ReDim archivos(1 To n): ReDim archivos2(1 To n)
    i = 0
    ...
    Next i
    For i = 1 To n
        archivos(i) = dArch(meses(i))             ' ruta completa (cierre)
        If dArch2.Exists(meses(i)) Then
            archivos2(i) = dArch2(meses(i))       ' respaldo: dia anterior
        Else
            archivos2(i) = vbNullString
        End If
    Next i
    ListarCierres = n
End Function

'--- Extrae una fecha de 8 digitos del nombre (AAAAMMDD o DDMMAAAA) --
Private Function ExtraerFecha8(nombre As String) As String
    Dim i As Long, corrida As Long, f8 As String
    Dim aa As Long, mm As Long, dd As Long
    corrida = 0
    For i = 1 To Len(nombre)
        If Mid$(nombre, i, 1) Like "#" Then
            ...
            If corrida >= 8 Then
                ...
                If aa >= 2015 And aa <= 2035 And mm >= 1 And mm <= 12 _
                ...
                If aa >= 2015 And aa <= 2035 And mm >= 1 And mm <= 12 _
                   And dd >= 1 And dd <= 31 Then
                    ExtraerFecha8 = Format$(aa, "0000") & Format$(mm, "00") & Format$(dd, "00")
                    Exit Function
                End If
            End If
        Else
...
'--- DIAGNOSTICO: lista en la hoja "Diag" todo archivo FMS que ve ----
'    y si lo reconoce o no. Para investigar meses que no cargan.
Public Sub DiagnosticoCarpeta()
    Dim fso As Object, ws As Worksheet, fila As Long
    Set fso = CreateObject("Scripting.FileSystemObject")
    On Error Resume Next
    Application.DisplayAlerts = False
    ThisWorkbook.Worksheets("Diag").Delete
    Application.DisplayAlerts = True
    On Error GoTo 0
    Set ws = ThisWorkbook.Worksheets.Add( _
        After:=ThisWorkbook.Worksheets(ThisWorkbook.Worksheets.Count))
    ws.Name = "Diag"
    ws.Cells(1, 1).Value = "Carpeta"
    ws.Cells(1, 2).Value = "Archivo"
    ws.Cells(1, 3).Value = "Reconocido"
    ...
    For r = LBound(rutas) To UBound(rutas)
        If CStr(rutas(r)) <> vbNullString Then
            ws.Cells(fila, 1).Value = "CARPETA: " & rutas(r)
            If fso.FolderExists(rutas(r)) Then
                ws.Cells(fila, 3).Value = "carpeta OK"
                ws.Rows(fila).Font.Bold = True
                fila = fila + 1
                DiagCarpeta fso.GetFolder(rutas(r)), ws, fila
            Else
                ws.Cells(fila, 3).Value = "NO EXISTE"
                ws.Rows(fila).Font.Bold = True
                fila = fila + 1
            End If
        End If
    Next r
    ws.Columns(1).ColumnWidth = 55
    ws.Columns(2).ColumnWidth = 40
    ws.Columns(3).ColumnWidth = 16
    ws.Cells.Font.Name = "Arial": ws.Cells.Font.Size = 8
    ws.Rows(1).Font.Bold = True
    MsgBox "Listados " & (fila - 2) & " archivos en la hoja 'Diag'. " & _
           "Busca los meses que faltan y revisa la columna Reconocido.", vbInformation
End Sub

Private Sub DiagCarpeta(fld As Object, ws As Worksheet, ByRef fila As Long)
    Dim f As Object, sf As Object, f8 As String
    For Each f In fld.Files
        If fila > 2000 Then Exit Sub
        If InStr(1, f.Name, "FMS", vbTextCompare) > 0 Then
            ws.Cells(fila, 1).Value = fld.Path
            ws.Cells(fila, 2).Value = f.Name
            f8 = ExtraerFecha8(f.Name)
            If f8 <> vbNullString And UCase$(f.Name) Like "*.XLS*" _
               And UCase$(f.Name) Like "FMS*" Then
                ws.Cells(fila, 3).Value = "OK -> " & Left$(f8, 6)
            Else
                ws.Cells(fila, 3).Value = "NO RECONOCIDO"
            End If
            fila = fila + 1
        End If
    Next f
    ...
    For Each sf In fld.SubFolders
        DiagCarpeta sf, ws, fila
    Next sf
    On Error GoTo 0
End Sub

'--- Recorre una carpeta y sus subcarpetas juntando los FMS ----------
Private Sub EscanearCarpeta(fld As Object, dArch As Object, dDia As Object, _
                            dArch2 As Object, dDia2 As Object)
    Dim f As Object, sf As Object
    Dim nom As String, f8 As String, ym As String, dd As Long
    For Each f In fld.Files
        nom = f.Name
        ' Tolerante: empieza con FMS, extension Excel, y una fecha de 8
        ' digitos en cualquier parte del nombre (AAAAMMDD o DDMMAAAA)
        If UCase$(nom) Like "FMS*" And UCase$(nom) Like "*.XLS*" Then
            ...
            If f8 <> vbNullString Then
                ...
                If Not dDia.Exists(ym) Then
                    dArch(ym) = f.Path: dDia(ym) = dd
                ElseIf dd > dDia(ym) Then         ' nuevo cierre: el anterior
                    dArch2(ym) = dArch(ym)        ' pasa a ser el respaldo
                    dDia2(ym) = dDia(ym)
                    dArch(ym) = f.Path: dDia(ym) = dd
                ElseIf dd < dDia(ym) Then
                    If (Not dDia2.Exists(ym)) Then
                        dArch2(ym) = f.Path: dDia2(ym) = dd
                    ElseIf dd > dDia2(ym) Then
                        dArch2(ym) = f.Path: dDia2(ym) = dd
                    End If
                End If
            End If
        End If
    Next f
    On Error Resume Next                          ' subcarpetas sin permiso: saltar
    For Each sf In fld.SubFolders
        EscanearCarpeta sf, dArch, dDia, dArch2, dDia2
    Next sf
    On Error GoTo 0
End Sub

'--- Abre un FMS_AAAAMMDD, lee F1/F2 y pega la columna del mes -------
'    Devuelve False si el archivo fallo (se anota y se continua)
Private Function CargarArchivo(wsD As Worksheet, ym As String, ruta As String, _
                               ByRef informe As String) As Boolean
...
FalloArchivo:
    ...
    informe = informe & nombre & " -> ERROR " & numErr & ": " & descErr & _
              " (archivo saltado)" & vbCrLf
End Function
...
Private Function ObtenerHojaDestino() As Worksheet
    ...
    If Not ws Is Nothing Then
        If Trim$(CStr(ws.Cells(2, 3).Value)) = vbNullString Then
            ws.Cells(2, 2).Value = "Carpeta(s) FMS (separar con ;) ->"
...
'   - Bloque 2 "Base 0": CADA diciembre = 0% (base del anio) y
'     dentro del anio crece compuesto; mes vacio arrastra. El grafico
'     muestra solo el anio en curso (desde el ultimo diciembre).
...
Public Sub CrearMonitor()
    ...
    For i = 0 To UBound(cat)
        ...
        For c = RPM_MES_INI To RPM_MES_FIN
            ...
            ' ---- bloque Base 0: CADA diciembre = 0% (base del anio) ----
            ' asi cada anio tiene su propia curva YTD, para toda la historia
            If ((c - RPM_MES_INI) Mod 12) = 0 Then
...
Public Sub ActualizarGraficosMeses()
    ...
    ' ventana del grafico: desde el diciembre anterior al ultimo dato
    Dim iniCol As Long, m As Long
    m = (lastCol - RPM_MES_INI) Mod 12
    If m = 0 Then                                 ' el ultimo dato ES diciembre:
        iniCol = lastCol - 12                     ' mostrar ese anio completo
    Else
        iniCol = lastCol - m
    End If
    If iniCol < RPM_MES_INI Then iniCol = RPM_MES_INI
    ...
    For Each co In ws.ChartObjects
        ...
        For s = 1 To co.Chart.SeriesCollection.Count
            With co.Chart.SeriesCollection(s)
                .XValues = ws.Range(ws.Cells(RPM_FECHA2, iniCol), _
                                    ws.Cells(RPM_FECHA2, lastCol))
                .Values = ws.Range(ws.Cells(base + s - 1, iniCol), _
...
Private Function IdxFMS(fila As Long, filaFecha As Long, refFecha As String) As String
    IdxFMS = "INDEX(FMS!$E$" & fila & ":$ZZ$" & fila & ",MATCH(" & refFecha & _
             ",FMS!$E$" & filaFecha & ":$ZZ$" & filaFecha & ",0))"
End Function

Private Function IdxFMSBlq(f1 As Long, f2 As Long, filaFecha As Long, _
                           refFecha As String) As String
    IdxFMSBlq = "INDEX(FMS!$E$" & f1 & ":$ZZ$" & f2 & ",0,MATCH(" & refFecha & _
                ",FMS!$E$" & filaFecha & ":$ZZ$" & filaFecha & ",0))"
End Function
...
Public Sub CrearCalculos()
    ...
    For c = RPM_MES_INI + 1 To RPM_MES_FIN
        ...
        For k = 0 To 2
            ...
            ws.Cells(73 + k, c).Formula2 = "=IFERROR(SUMPRODUCT(" & pes & _
                "*IFERROR(" & ret & "+0,0))/SUMPRODUCT(" & pes & "*(" & ret & _
                "<>"""")),"""")"
...
Public Sub CrearResumen()
    ...
    For b = 0 To 1
        ...
        Dim hdrs As Variant
        hdrs = Array("Instrumento", "", "", "", "YTD", "1M", "3M", "6M", _
                     "Pos F1 (MM)", "Pos F2 (MM)", "% F1", "% F2", "")
        For k = 0 To UBound(hdrs)
            ws.Cells(rIni - 1, 2 + k).Value = hdrs(k)
        Next k
        For k = 0 To 2                             ' anos rodantes (siguen a C3)
            ws.Cells(rIni - 1, 3 + k).Formula = "=IFERROR(YEAR($C$3)-" & (3 - k) & ","""")"
        Next k
        ws.Cells(rIni - 1, 14).Formula = "=IFERROR(""Anualiz. ""&YEAR($C$3),""Anualiz."")"
        EncabezadoFila ws, rIni - 1, 2, 14

        For i = 0 To 5
            fi = idx0 + i: r = rIni + i: rr = RPM_R_INI + fi
            ws.Cells(r, 2).Formula = "='" & HOJA_RPM & "'!B" & rr
            For k = 0 To 2                     ' tres anos cerrados anteriores
                ws.Cells(r, 3 + k).Formula = "=IFERROR(INDEX(" & src & "$" & _
                    ...
                    R2_FILA_INI & ":$" & R2_COL_SBS & "$" & R2_FILA_FIN & ",0),MATCH(YEAR($C$3)-" & (3 - k) & _
                    ...
                    ":$" & R2_COL_SBS & "$" & R2_FILA_FIN & ",0),MATCH(TEXT(YEAR($C$3)-" & (3 - k) & ",""0"")," & src & _
                    "$" & R2_COL_INI & "$" & R2_FILA_FECHAS & ":$" & R2_COL_FIN & "$" & _
                    R2_FILA_FECHAS & ",0)),""""))"
            Next k
            ws.Cells(r, 6).Formula2 = "=IF($P$3="""","""",IFERROR(EXP(SUMPRODUCT(LN(1+" & _
                "IFERROR(OFFSET('" & HOJA_RPM & "'!$D$4," & (fi + 1) & _
                ",$P$3-MONTH($C$3)+1,1,MONTH($C$3))+0,0))))-1,""""))"
            ws.Cells(r, 7).Formula = "=IF($P$3="""","""",IF(INDEX('" & HOJA_RPM & _
                "'!$E$" & rr & ":$" & LF & "$" & rr & ",$P$3)="""","""",INDEX('" & HOJA_RPM & _
                "'!$E$" & rr & ":$" & LF & "$" & rr & ",$P$3)))"
            ws.Cells(r, 8).Formula2 = "=IF(OR($P$3="""",$P$3<3),"""",IFERROR(EXP(" & _
                "SUMPRODUCT(LN(1+IFERROR(OFFSET('" & HOJA_RPM & "'!$D$4," & (fi + 1) & _
                ",$P$3-2,1,3)+0,0))))-1,""""))"
            ws.Cells(r, 9).Formula2 = "=IF(OR($P$3="""",$P$3<6),"""",IFERROR(EXP(" & _
                "SUMPRODUCT(LN(1+IFERROR(OFFSET('" & HOJA_RPM & "'!$D$4," & (fi + 1) & _
                ",$P$3-5,1,6)+0,0))))-1,""""))"
            ...
            ws.Cells(r, 13).Formula = "=IFERROR(" & IdxFMS(22 + fi, 21, "$C$3") & "/" & _
                IdxFMS(34, 21, "$C$3") & ","""")"
            ws.Cells(r, 14).Formula = "=IF($P$3="""","""",IFERROR(F" & r & _
                "/COUNT(OFFSET('" & HOJA_RPM & "'!$D$4," & (fi + 1) & _
                ",$P$3-MONTH($C$3)+1,1,MONTH($C$3)))*12,""""))"
            ws.Range(ws.Cells(r, 3), ws.Cells(r, 9)).NumberFormat = "0.00%"
            ws.Range(ws.Cells(r, 10), ws.Cells(r, 11)).NumberFormat = "#,##0.0"
            ws.Range(ws.Cells(r, 12), ws.Cells(r, 14)).NumberFormat = "0.00%"
        Next i
